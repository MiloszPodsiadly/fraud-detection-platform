...
class ProducerEngineIntelligenceFailureIsolationTest {
    ...
    void aggregationFailureReturnsBaseEventWithoutNestedField() throws Exception {
        var emission = new EngineIntelligenceEmissionService(new EngineIntelligenceEmissionProperties(true));
        var intelligence = emission.mapIfEnabled(() -> {
            throw new IllegalStateException("raw-secret-must-not-leak");
        });
    ...
    void mapperFailureReturnsEmptyOptional() {
        ...
        var emission = new EngineIntelligenceEmissionService(new EngineIntelligenceEmissionProperties(true), mapper);

        assertThat(emission.mapIfEnabled(() -> aggregation)).isEmpty();
    }
