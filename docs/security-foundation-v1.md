...
- `GET /api/v1/audit/integrity/external/coverage` requires `audit:verify`, accepts optional `from_position`, is per-instance rate-limited, and returns bounded operational coverage: `latest_local_position`, `latest_external_position`, `position_lag`, `time_lag_seconds`, `missing_ranges`, `truncated`, `limit`, `local_ahead_of_external`, `required_publication_failures`, `local_status_unverified`, `recovered_count`, and `unrecovered_count`. `limit` defaults to 100 and is capped at 100. Coverage uses the publication-status repository as the local bounded index and does not perform one external object read per chain position. `truncated=true`, `local_ahead_of_external=true`, or non-zero required/unverified/unrecovered counts means the bounded window cannot prove complete coverage; there is no unbounded scan or export.
- `GET /system/trust-level` is live-state posture, not config-only posture. It returns `NONE`, `BEST_EFFORT`, `FDP24_CONFIGURED`, `FDP24_HEALTHY`, or `FDP24_DEGRADED` plus coverage, witness, signature policy, missing range, required-failure, local-status-unverified, and post-commit audit degradation signals. `FDP24_HEALTHY` requires enabled+required+fail-closed publication, verified witness capabilities, healthy bounded coverage, no required publication failures, no local status gaps, no observed post-commit audit degradation, and required signature health when configured.
