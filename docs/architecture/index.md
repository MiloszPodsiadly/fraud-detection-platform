...
| --- | --- | --- | --- |
| [Controlled engine intelligence producer emission rollout](engine_intelligence_producer_emission_rollout.md) | FDP-94 disabled-by-default producer capability boundary only | Specific emission flag, optional public mapper boundary, failure isolation, rollback, and live-runtime limitation | Claiming live orchestrator migration, default production emission, alert-service projection, persistence, API/UI exposure, or final decisioning |
...
| [Public engine intelligence event contract](public_engine_intelligence_event_contract.md) | FDP-92 contract-only foundation | Optional bounded public engine-intelligence DTOs, compatibility rules, field allowlist, and unwired mapping design | Claiming producer wiring, alert-service projection, API/UI consumption, or final decisioning |
...
| [Internal fraud scoring orchestrator](fraud_scoring_orchestrator.md) | FDP-89 foundation with FDP-90 internal runtime hardening | Internal `FraudScoringOrchestrator`, deterministic adapter execution, bounded per-engine failure containment, deadline enforcement, and runtime isolation | Claiming production scoring migration, Kafka event/API/UI/projection integration, final decisioning, or public `engineResults[]` |
