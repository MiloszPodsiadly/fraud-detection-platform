...
- Analyst decision responses expose `operation_status`: `REJECTED_BEFORE_MUTATION`, `COMMITTED_EVIDENCE_PENDING`, `COMMITTED_EVIDENCE_INCOMPLETE`, or `COMMITTED_FULLY_ANCHORED`. `REJECTED_BEFORE_MUTATION` maps to HTTP 503 with no command response body and means mutation was not executed. `COMMITTED_EVIDENCE_PENDING` means the business decision and outbox record committed but full evidence completion has not been persisted yet. `COMMITTED_EVIDENCE_INCOMPLETE` means the decision and outbox record committed but post-commit evidence anchoring degraded; it is not a rollback and is counted by durable `audit_degradation_events` plus `fraud_platform_post_commit_audit_degraded_total`. `COMMITTED_FULLY_ANCHORED` is reserved for verified local success audit anchor, external `PUBLISHED` state, and clean required signature policy; it is not inferred from a plain repository save. Post-commit degradation must not be reported as mutation-not-executed.
...
- `FraudDecisionEvent` includes `delivery=AT_LEAST_ONCE` and `dedupeKey=eventId`; consumers MUST deduplicate by event id. `PUBLISH_CONFIRMATION_UNKNOWN` means Kafka publish may have succeeded but alert-service could not durably mark the outbox row as published, so it is not retried automatically without `audit:verify` reconciliation. Operators may resolve an unknown confirmation as `PUBLISHED` or `RETRY_REQUESTED`; retry keeps the same event id and dedupe key.
...
- `GET /system/trust-level` requires backend-enforced `audit:verify`. It returns live posture, not config-only posture, and clients must treat any unresolved degradation, terminal/unknown outbox state, missing range, local-status-unverified state, or required publication failure as `FDP24_DEGRADED`, not healthy.
- `GET /api/v1/audit/degradations` and `POST /api/v1/audit/degradations/{auditId}/resolve` require `audit:verify`. Resolution records `resolved_at`, `resolved_by`, bounded reason, bounded evidence reference, and a resolution audit event.
- `GET /api/v1/decision-outbox/unknown-confirmations` and `POST /api/v1/decision-outbox/unknown-confirmations/{alertId}/resolve` require `audit:verify`. Unknown confirmation can be resolved only as `PUBLISHED` or `RETRY_REQUESTED`; retry keeps the same event id and dedupe key.
