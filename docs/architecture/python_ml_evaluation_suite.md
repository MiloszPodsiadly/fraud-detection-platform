...
Status: Offline evaluation foundation. FDP-124/FDP-126 is the active path for feedback dataset evaluation and Shadow Performance contract artifacts. The previous FDP-102/FDP-103 executable path has been removed.
...
FDP-124 adds a separate offline/internal path for feedback dataset JSONL. FDP-124 consumes feedback dataset `DATASET_RECORD` rows from the schema-backed envelope at `docs/schemas/feedback_dataset_record.schema.json`.
...
The feedback dataset/FDP-124 label mapping is limited to:
...
feedback dataset currently supplies risk and score buckets rather than raw numeric ML scores. FDP-124 therefore uses documented
...
FDP-124 accepts feedback dataset pseudonymous input references only for parsing and deterministic ordering. Reports are
...
feedback dataset/FDP-124/FDP-126 path described below.
...
FDP-124 disagreement rows may include `decisionReasonCodes` because feedback dataset validates them as bounded machine-code
...
feedback dataset/FDP-124 Platform Recommendation Evaluation Card v1 is the single executable Platform Recommendation Evaluation Card implementation. It is a local/internal governance
...
FDP-124 `disagreementSummary` into Platform Recommendation Evaluation Card v1, does not read the raw feedback dataset, and does not expose raw IDs
