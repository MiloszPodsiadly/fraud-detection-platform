...
External audit anchoring is disabled by default with `app.audit.external-anchoring.enabled=false`. FDP-20 supports `disabled` and `local-file`; FDP-22 adds `object-store` as a pluggable immutable-storage boundary behind `ExternalAuditAnchorSink`. The legacy `external-object-store` placeholder is rejected; use `app.audit.external-anchoring.sink=object-store`. When enabled with `app.audit.external-anchoring.sink=local-file`, alert-service publishes local audit anchors to a local verification JSONL sink outside MongoDB. Publication is idempotent by `local_anchor_id`; duplicate publication does not overwrite the existing external anchor. External sink failures are logged and counted, but they do not block durable audit writes.
...
FDP-22 provides an object-store sink contract for external persistence outside MongoDB. Anchor objects are written under `audit-anchors/{partition_key}/{chain_position}.json` using a bounded payload containing `local_anchor_id`, `partition_key`, `chain_position`, `event_hash`, `previous_event_hash`, and `created_at`. The application never exposes delete/update paths for external anchors and enforces logical immutability by reading an existing object before write: identical content is treated as idempotent, different content for the same key fails with an explicit mismatch.

Object-store mode is strict. `app.audit.external-anchoring.sink=object-store` requires `bucket`, `prefix`, `region` or `endpoint`, credentials, and a configured object-store client adapter. Missing configuration fails startup; there is no silent fallback to local-file. Verification uses bounded reads and exact `partition_key + chain_position` lookup when the sink supports it, with `MATCH`, `MISSING`, and `MISMATCH` mapped to the existing `VALID`, `PARTIAL`, and `INVALID` external integrity states.

FDP-22 provides a logical immutable boundary only. Immutability depends on object-store infrastructure configuration such as object lock, retention policy, bucket policy, or versioning. It does not provide a WORM guarantee by itself, legal notarization, legal non-repudiation, compliance certification, SIEM integration, or regulator-ready archive.
...
- `SIGNED_ATTESTATION`: external anchor verification is valid and attestation signing is production-ready.
