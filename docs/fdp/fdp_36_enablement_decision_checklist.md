...
| Proof summary artifact | `fdp36-proof-summary.md` uploaded by CI | `PASS`, `FAIL`, `NOT_RUN` | `TO_BE_FILLED_BY_CI_OR_RELEASE_OWNER` |
...
| Inspection endpoint auth | unauthenticated and unauthorized analyst access rejected; `FRAUD_OPS_ADMIN` and `AUDIT_VERIFY` inspection access proven | `PASS`, `FAIL`, `NOT_RUN` | `TO_BE_FILLED_BY_CI_OR_RELEASE_OWNER` |
| Inspection sensitive response review | no raw idempotency key, request hash, intent hash, actor id, lease owner, correlation id, token, path, stack trace, or raw resource id | `PASS`, `FAIL`, `NOT_RUN` | `TO_BE_FILLED_BY_CI_OR_RELEASE_OWNER` |
| Inspection read audit | regulated mutation inspection read audit verified or production-gate item recorded | `PASS`, `PRODUCTION_GATE`, `NOT_RUN` | `TO_BE_FILLED_BY_CI_OR_RELEASE_OWNER` |
| Inspection rate limit | per-actor/IP inspection rate limiting verified | `PASS`, `FAIL`, `NOT_RUN` | `TO_BE_FILLED_BY_CI_OR_RELEASE_OWNER` |
