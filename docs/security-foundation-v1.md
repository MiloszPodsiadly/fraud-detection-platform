...
- Audit trust attestation signing is controlled by `app.audit.trust-attestation.signing.mode=disabled|local-dev|kms-ready`. `local-dev` is for development and verification only, provides integrity metadata only, does not add external trust, and is rejected in prod-like profiles. `kms-ready` fails startup until a real KMS/HSM adapter exists; the server must not silently fall back to unsigned attestation when signing mode is enabled.
