...
| Model Card v1 | FDP-104 diagnostic-only governance artifact that consumes FDP-103 aggregate evaluation reports plus bounded model metadata. It validates bucket-ordered offline metrics, treats analyst labels as evaluation signals rather than ground truth, and keeps approvedFor limited to SHADOW/COMPARE diagnostic modes. It does not read production DBs, raw payloads, raw feature vectors, mutate model artifacts, retrain models, promote models, recommend thresholds, change scoring, expose API/UI, create dashboards, recommend analyst actions, approve/decline/block, or authorize payments. |
| Shadow Performance Summary v1 | FDP-105 offline diagnostic summary derived only from validated FDP-104 Model Card v1 aggregate fields. It includes population context for offline diagnostic metrics so sample size travels with precision, recall, and false-positive-rate values. It does not recompute metrics, read FDP-102/FDP-103 raw sources, promote models, recommend thresholds, approve production decisioning, authorize payments, mutate scoring or model artifacts, expose API/UI, create dashboards, emit Kafka events, or recommend analyst actions. |
