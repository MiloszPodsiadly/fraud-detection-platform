...
class FeedbackDatasetEvaluationCardSchemaTest(unittest.TestCase):
    ...
    def test_recordLimitUsesFeedbackDatasetMaximum(self):
        self.assertEqual(MAX_FEEDBACK_DATASET_RECORDS, MAX_COUNT_VALUE)
        self._assert_rejected(evaluationEvidence=valid_evaluation_evidence(
            recordsEvaluated=MAX_FEEDBACK_DATASET_RECORDS + 1,
            positiveClassCount=MAX_FEEDBACK_DATASET_RECORDS + 1,
    ...
    def test_canonicalTimestampMatrixRejectedWithDomainException(self):
        for value in INVALID_CANONICAL_TIMESTAMPS:
            with self.subTest(value=value):
                with self.assertRaises(FeedbackDatasetEvaluationCardValidationError) as caught:
