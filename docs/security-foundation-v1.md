...
- `app.trust-authority.external-anchoring.enabled=false` by default. The disabled FDP-24 hook can read and log the local audit head when explicitly enabled, but there is no real external anchor publisher in FDP-23. It must not be documented or interpreted as external notarization, WORM storage, or independent third-party trust.
