...
- Alert-service exposes `GET /api/v1/audit/trust/keys`, protected by backend `audit:verify`, as a bounded public verification key proxy. It returns `key_id`, `algorithm`, `public_key`, `valid_from`, `valid_until`, and `status` (`ACTIVE`, `RETIRED`, or `REVOKED`); it never returns private keys.
...
- The trust authority audits every `/sign` and `/verify` call before returning, and audit write failure fails the request. Audit records contain bounded caller identity, purpose, payload hash, key id, result, reason code, and timestamp only.
- `/sign` requires a per-service HMAC-signed request plus purpose authorization. Service identity headers are bound into the HMAC signature and are not trusted by themselves. Default local policy permits only `alert-service` for `AUDIT_ANCHOR`; `fraud-scoring-service` has no signing purpose. Prod-like profiles require explicit per-caller non-default HMAC secrets. Unauthorized purpose returns `403`, invalid credentials return `401`, replay mismatch returns `409`, and per-verified-service rate limit exceed returns `429`.
- The trust authority supports a minimal local key lifecycle registry: `ACTIVE` signs new anchors, `RETIRED` verifies historical signatures, and `REVOKED` or unknown keys fail verification. Verification enforces `signed_at` within key validity windows. Prod-like profiles reject default local HMAC credentials, require `signing-required=true`, and reject generated ephemeral signing keys. Per-service HMAC is a local-first service identity guard; it is not mTLS, not channel binding, not enterprise service identity, and not a zero-replay guarantee.
