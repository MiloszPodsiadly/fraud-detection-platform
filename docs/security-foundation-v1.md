...
- FDP-25 Phase 1 introduces a regulated mutation coordinator for submit-decision writes. The coordinator owns mandatory idempotency, command state, atomic command execution claim/lease, `ATTEMPTED` audit, business decision persistence with same-document outbox, `SUCCESS` audit, response snapshot replay, recovery-required states, and degraded evidence state. `IN_PROGRESS`, `RECOVERY_REQUIRED`, and `COMMIT_UNKNOWN` are explicit HTTP 202 command states. `COMMITTED_FULLY_ANCHORED` is async-only and not returned by the synchronous request path. FDP-25 Phase 1 is a saga-style recoverable command state machine, not distributed ACID across MongoDB, external witnesses, and Kafka; it detects and exposes post-commit audit degradation but does not transactionally eliminate it. External evidence remains asynchronous and reconciled. Fraud-case update and governance writes remain future coordinator migrations.
