...
- External anchor publication is disabled by default (`app.audit.external-anchoring.publication.enabled=false`). Legacy `app.audit.external-anchoring.enabled=true` is deprecated, remains backward-compatible, maps to required fail-closed publication, and emits a startup warning; new configuration should use `publication.enabled`, `publication.required`, and `publication.fail-closed`; `fail-closed=true` requires `required=true`. Supported sink names are `disabled`, `local-file`, and `object-store`; the legacy `external-object-store` placeholder is rejected. When publication is enabled or required, startup requires a verified external witness and rejects fake publishers including `disabled`, `noop`, `local-file`, `in-memory`, and `same-database`. Local-file is only a development verification artifact and cannot satisfy enabled or required external anchoring. FDP-22 `object-store` writes new anchors to a configured external object-store adapter using `audit-anchors/{encoded_partition_key}/{chain_position_padded}.json`, where `chain_position_padded` is a 20-digit zero-padded number and `encoded_partition_key` is deterministic base64url without padding. Legacy non-padded keys remain readable without a migration job. Object-store mode requires explicit bucket/prefix/region-or-endpoint/credentials/client configuration, performs startup readiness checking by default, and fails startup if configuration or readiness is incomplete. Required external publication is not transactional with local durable append; success events are business-effective only after their local anchor is externally confirmed, while failed required publication records an `ABORTED_EXTERNAL_ANCHOR_REQUIRED` compensation event related to the affected event and coverage exposes `local_ahead_of_external=true` plus `required_publication_failures`.
...
- Companion publication status records track external publication status fields for later operational inspection. Success records require `external_published=true`, `external_publication_status=PUBLISHED`, `external_published_at`, `external_sink_type`, `external_object_status`, `head_manifest_status`, `local_tracking_status`, and `local_anchor_status`; unverified records set `external_published=false`, `external_publication_status=UNVERIFIED`, and a bounded `last_external_publish_failure_reason`; failure records set `external_published=false`, `external_publication_status=FAILED`, and may include `last_external_publish_failure_reason`. If status persistence fails after external publication/read-back, `external_anchor_status_persistence_failed_total{sink}` is incremented and the write path fails closed in required mode with `STATUS_PERSISTENCE_FAILED_AFTER_EXTERNAL_PUBLISH`; the system must not report a clean local `PUBLISHED` state. Audit read and evidence export expose `compensated`, `superseded_by_event_id`, `business_effective`, and `related_event_id`; `ATTEMPTED` plus `ABORTED_EXTERNAL_ANCHOR_REQUIRED` is never final business success. If required external publication fails after local anchor creation, the status is `LOCAL_ANCHOR_CREATED_EXTERNAL_REQUIRED_FAILED` with `external_required=true`, `local_anchor_status=CREATED`, and `external_object_status=FAILED` or `UNAVAILABLE`. Default reconciliation scans a bounded recent local window for missing status, `LOCAL_STATUS_UNVERIFIED`, and required-failure states. Large or old gaps require explicit bounded range reconciliation by `partition_key`, `from_position`, `to_position`, and `limit`; reconciliation verifies exact `partition_key + chain_position` and records recovered/still-unverified/missing/invalid results. These fields are not part of the event hash chain and updating them does not mutate audit events or local anchor records.
