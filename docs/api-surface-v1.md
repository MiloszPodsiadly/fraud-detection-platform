...
- `/sign` requires token plus caller service identity and purpose authorization. Default local policy permits only `alert-service` for `AUDIT_ANCHOR`; `fraud-scoring-service` has no signing purpose. Prod-like profiles require explicit per-caller non-default tokens. Unauthorized purpose returns `403`, invalid credentials return `401`, and per-caller rate limit exceed returns `429`.
- The trust authority supports a minimal local key lifecycle registry: `ACTIVE` signs new anchors, `RETIRED` verifies historical signatures, and `REVOKED` or unknown keys fail verification. Verification enforces `signed_at` within key validity windows. Prod-like profiles reject the default shared token, require `signing-required=true`, and reject generated ephemeral signing keys. Shared-token auth is a local development guard only; it is not mTLS, not channel binding, and not enterprise service identity.
