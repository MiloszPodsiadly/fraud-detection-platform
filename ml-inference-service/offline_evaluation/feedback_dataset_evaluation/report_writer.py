...
def report_json(report: dict[str, Any]) -> str:
    ...
    if safe_report.get("reportType") not in REPORT_TYPES:
        raise ValueError("reportType must be a supported feedback dataset evaluation report type")
...
def write_feedback_dataset_evaluation_reports(
...
def evaluation_run_markdown(summary: dict[str, Any]) -> str:
    ...
    if not isinstance(warnings, list) or not all(isinstance(item, str) for item in warnings):
        raise ValueError("warnings must contain strings")
    lines = [
        "# Feedback Dataset Offline Evaluation Run",
        "",
