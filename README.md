...
- each durable audit event is linked into a per-partition SHA-256 hash chain using `previous_event_hash`
- each inserted audit event creates an append-only chain anchor in `audit_chain_anchors`; the anchor stores the latest event hash, chain position, partition key, and hash algorithm
- bounded integrity verification checks event hashes, previous-hash continuity, schema version, hash algorithm, fork indicators, and latest anchor-to-chain-head consistency
- scheduled verification checks the latest bounded window automatically and records low-cardinality metrics; it logs visible errors on violations and never repairs data
...
Audit integrity verification is available through `GET /api/v1/audit/integrity` and requires `audit:read`. The endpoint is read-only, bounded (`limit` default 100, max 500), supports optional inclusive `from`/`to` and bounded `source_service=alert-service`, and returns `VALID`, `INVALID`, `PARTIAL`, or `UNAVAILABLE`. It verifies stored hashes, previous-hash continuity, schema version, hash algorithm, fork indicators, and latest anchor consistency without mutating or repairing data. Integrity checks are themselves audited with `VERIFY_AUDIT_INTEGRITY`.
