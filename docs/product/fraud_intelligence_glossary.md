...
| Model Card v1 | FDP-104 offline governance artifact that consumes FDP-103 aggregate evaluation reports plus bounded model metadata. It is diagnostic-only and does not read production DBs, raw payloads, raw feature vectors, mutate model artifacts, retrain models, promote models, recommend thresholds, change scoring, expose API/UI, create dashboards, recommend analyst actions, approve/decline/block, or authorize payments. |
