...
| Contract summary | Human-readable companion to code/API contracts. | `docs/api/status_truth_table.md`, `docs/configuration/configuration_guide.md` | Must match enum/config/controller behavior or explicitly call out gaps. |
| FDP branch evidence | Branch-specific proof, handoff, merge-gate, or readiness documents. | `docs/fdp/fdp_*.md`, `docs/**/fdp_*.md` | Preserve for traceability; do not treat as current truth when superseded. |
...
| Supporting local docs | Local setup or service identity material. | `deployment/service-identity/README.md` | Must label local/dev material as local/dev and avoid production claims. |
...
- [API status truth table](api/status_truth_table.md)
...
## FDP Evidence Handling

FDP branch documents are retained as trace evidence for the branch that produced them. They may contain
older assumptions, narrower proof scope, or limitations that were superseded by later FDP work.

FDP branch documents must not be read as current production truth unless a current source-of-truth document
...
If a branch document conflicts with a current source-of-truth document, the current source-of-truth
document wins. If two FDP branch documents conflict, the later FDP document is the more recent evidence unless
...
- Current documentation filenames use `snake_case` names under domain folders.
...
- FDP branch documents can contain older scope, older risk statements, or template placeholders.
- FDP branch documents are retained as trace evidence and are not automatically rewritten into current truth.
