...
- `app.audit.trust-attestation.signing.mode=disabled|local-dev|kms-ready`. `local-dev` is for local development and verification only, provides integrity metadata only, does not provide external trust, and is rejected in prod-like profiles. `kms-ready` fails startup until a real KMS/HSM adapter is supplied.
