...
`GET /system/trust-level` exposes the current deployment posture: `guarantee_level`, publication flags, fail-closed flag, and `external_anchor_strength=UNSIGNED_EXTERNAL|SIGNED_EXTERNAL`. `BEST_EFFORT` must not be marketed as FDP-24 fail-closed mode. Startup logs `FDP-24 FAIL-CLOSED MODE ACTIVE` only when required publication and fail-closed mode are both enabled.
