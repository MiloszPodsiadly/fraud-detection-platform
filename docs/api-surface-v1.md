...
- Fraud-case update writes still use the FDP-24 guarded audit pattern. Submit decision writes now use the FDP-25 Phase 1 regulated mutation coordinator.
- `POST /api/v1/alerts/{alertId}/decision` requires `X-Idempotency-Key`, stores a `regulated_mutation_commands` record with canonical intent fields and an idempotency-key hash, atomically claims command execution with a bounded lease, writes durable `ATTEMPTED` audit before business mutation, and in `app.regulated-mutations.transaction-mode=REQUIRED` commits command local state, alert business state, `transactional_outbox_records`, response snapshot, and local commit marker in one Mongo transaction. Phase audits use deterministic request ids `regulated_command_id:ATTEMPTED|SUCCESS|FAILED` so recovery can bind existing evidence without duplicating success audit events. `SUCCESS` audit, Kafka publication, external anchoring, trust-authority signature verification, and evidence confirmation are outside the local ACID transaction and are coordinated idempotently. The same key and payload replay the stored response snapshot; the same key with a different payload returns 409; a missing key returns 400.
- Analyst decision responses expose `operation_status`: `REJECTED_BEFORE_MUTATION`, `IN_PROGRESS`, `RECOVERY_REQUIRED`, `COMMIT_UNKNOWN`, `COMMITTED_EVIDENCE_PENDING`, `COMMITTED_EVIDENCE_CONFIRMED`, or `COMMITTED_EVIDENCE_INCOMPLETE`. `REJECTED_BEFORE_MUTATION` maps to HTTP 503 with no command response body and means mutation was not executed. `IN_PROGRESS`, `RECOVERY_REQUIRED`, and `COMMIT_UNKNOWN` map to HTTP 202 because the command is accepted but not safely reportable as a stable completed result. `COMMITTED_EVIDENCE_PENDING` means the decision and outbox record committed and local `SUCCESS` audit was recorded, while evidence promotion remains pending. `COMMITTED_EVIDENCE_CONFIRMED` means local commit marker, success audit, and authoritative `PUBLISHED` transactional outbox evidence are present; it is not external WORM/notarization proof. `COMMITTED_EVIDENCE_INCOMPLETE` means the decision and outbox record committed but local success audit/evidence completion degraded; it is not a rollback and is counted by durable `audit_degradation_events` plus `fraud_platform_post_commit_audit_degraded_total`. `COMMITTED_FULLY_ANCHORED` is intentionally not exposed by the synchronous request path.
...
- FDP-26A is `FDP-26A - Local Transactional Outbox & Evidence Confirmation Foundation`. It is a local Mongo ACID boundary plus transactional outbox delivery foundation. `transactional_outbox_records` are authoritative; alert embedded outbox fields are projection/cache compatibility only. It is not distributed ACID across MongoDB, Kafka, trust-authority signing, and external witnesses; it is not exactly-once delivery, pre-commit/finalize evidence-gated commit, legal notarization, WORM storage, SIEM integration, certified archive evidence, or business rollback after a visible commit. `SUCCESS` audit and external evidence remain outside the local Mongo transaction unless a future phase changes the commit model.
- Manual outbox confirmation resolution is operator-attested, not broker-verified and not dual-control in FDP-26A. It requires `X-Idempotency-Key`, reason, actor identity, and structured evidence. `PUBLISHED` requires broker-offset or equivalent confirmation evidence, but the platform does not independently verify broker state in FDP-26A.
...
Not implemented in FDP-26A. Candidate future states: `PENDING_COMMAND`, `ATTEMPT_EVIDENCE_READY`, `COMMIT_EVIDENCE_READY`, `FINALIZING`, `COMMITTED_LOCAL`, `EVIDENCE_CONFIRMED`, and `REJECTED_EVIDENCE_UNAVAILABLE`.

### FDP-28 - Trust Incident & Operations Control Plane

Not implemented in FDP-26A. Candidate future scope: `TrustIncidentDocument`, dedupe fingerprint, `OPEN`, `ACK`, `MITIGATING`, `RESOLVED`, `FALSE_POSITIVE`, dedicated authorities, and durable lifecycle.
