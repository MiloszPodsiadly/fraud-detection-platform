...
Status: current ML governance source of truth.

## Scope

This document describes the bounded ML governance layer for `ml-inference-service`.
...
| Topic | Section |
| --- | --- |
| Runtime governance endpoints | [Current Capabilities](#current-capabilities) and [Endpoint Summary](#endpoint-summary) |
| Reference and inference profile semantics | [Reference Profile](#reference-profile) and [Inference Profile](#inference-profile) |
| Persistence model | [Governance Snapshot Persistence](#governance-snapshot-persistence) |
| Model lifecycle visibility | [Model Lifecycle Visibility](#model-lifecycle-visibility) |
| Metrics and label policy | [Metrics](#metrics) |
| Operator response | [Incident Playbook](#incident-playbook) |
| Unsupported claims | [Limitations](#limitations) and [Out Of Scope](#out-of-scope) |
...
The current ML API surface and backward-compatibility rules are tracked in `docs/api/api_surface_v1.md`; the OpenAPI reference is `docs/openapi/ml_inference_service.openapi.yaml`. Governance and scoring success responses keep their existing top-level fields. Error responses use the platform `timestamp/status/error/message/details` envelope.
