...
class AuthorizationRulesCoverageTest extends AbstractSecurityRouteBoundaryWebMvcTest {
    ...
    void transactionRoutesRequireExplicitTransactionAuthority() throws Exception {
        ...
        expectSecurityLayerDoesNotReject(get("/api/v1/transactions/scored/txn-1/engine-intelligence/feedback")
                .with(userWith(AnalystAuthority.ENGINE_INTELLIGENCE_FEEDBACK_READ)));
        ...
        expectDenied(get("/api/v1/transactions/scored/txn-1/engine-intelligence/feedback")
                .with(userWith(AnalystAuthority.TRANSACTION_MONITOR_READ)));
        expectDenied(get("/api/v1/transactions/scored/txn-1/engine-intelligence/feedback")
                .with(userWith(AnalystAuthority.ENGINE_INTELLIGENCE_FEEDBACK_WRITE)));
        expectDenied(get("/api/v1/transactions/scored/txn-1/engine-intelligence/feedback")
                .with(userWith(AnalystAuthority.FRAUD_CASE_READ)));
        ...
        expectDenied(get("/api/v1/transactions/scored/txn-1/engine-intelligence/feedback/not-real")
                .with(userWith(AnalystAuthority.ENGINE_INTELLIGENCE_FEEDBACK_READ)));
    ...
    void suspiciousTransactionRoutesRequireExplicitSuspiciousTransactionAuthority() throws Exception {
        ...
        expectSecurityLayerDoesNotReject(get("/internal/suspicious-transactions/suspicious-1")
                .with(userWith(AnalystAuthority.SUSPICIOUS_TRANSACTION_READ)));
        expectSecurityLayerDoesNotReject(get("/internal/suspicious-transactions/suspicious-1/linked-alert")
                .with(userWith(AnalystAuthority.SUSPICIOUS_TRANSACTION_READ, AnalystAuthority.ALERT_READ)));
        expectDenied(get("/internal/suspicious-transactions/suspicious-1/linked-alert")
                .with(userWith(AnalystAuthority.SUSPICIOUS_TRANSACTION_READ)));
        expectDenied(get("/internal/suspicious-transactions/suspicious-1/linked-alert")
                .with(userWith(AnalystAuthority.ALERT_READ)));
