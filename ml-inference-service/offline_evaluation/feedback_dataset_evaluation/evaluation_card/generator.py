...
def _validate_manifest(manifest: dict[str, Any], summary: dict[str, Any], artifact_bytes: dict[str, bytes]) -> None:
    ...
    for item in files:
        if not isinstance(item, dict):
            raise FeedbackDatasetEvaluationCardValidationError("manifest file entries must be objects")
        ...
        if name not in EXPECTED_EVALUATION_ARTIFACT_FILENAMES:
            raise FeedbackDatasetEvaluationCardValidationError("manifest lists unsupported artifact")
        if name in seen_names:
            raise FeedbackDatasetEvaluationCardValidationError("manifest contains duplicate artifact")
        seen_names.add(name)
        expected_bytes = artifact_bytes[name]
        sha256 = item["sha256"]
        size_bytes = item["sizeBytes"]
        if not isinstance(sha256, str) or re.fullmatch(r"[a-f0-9]{64}", sha256) is None:
            raise FeedbackDatasetEvaluationCardValidationError("manifest file sha256 must be lowercase hex")
        if isinstance(size_bytes, bool) or not isinstance(size_bytes, int) or size_bytes < 0:
            raise FeedbackDatasetEvaluationCardValidationError("manifest file sizeBytes must be a non-negative integer")
        if sha256 != hashlib.sha256(expected_bytes).hexdigest():
            raise FeedbackDatasetEvaluationCardValidationError(f"{name} hash mismatch")
...
def _validate_summary(summary: dict[str, Any]) -> None:
    ...
    if summary.get("metricsSubject") != METRICS_SUBJECT:
        raise FeedbackDatasetEvaluationCardValidationError("evaluation summary metricsSubject unsupported")
    if summary.get("metricBasis") != METRIC_BASIS:
        raise FeedbackDatasetEvaluationCardValidationError("evaluation summary metricBasis unsupported")
    _normalize_timestamp(summary.get("generatedAt"), "evaluation summary generatedAt")
    ...
    if dataset_metadata.get("datasetVersion") != EXPECTED_DATASET_VERSION:
        raise FeedbackDatasetEvaluationCardValidationError("datasetVersion unsupported")
    if dataset_metadata.get("timeBasis") != EXPECTED_DATASET_TIME_BASIS:
        raise FeedbackDatasetEvaluationCardValidationError("dataset time basis unsupported")
    ...
    if missing_metrics:
        raise FeedbackDatasetEvaluationCardValidationError(f"qualityMetrics missing required fields: {', '.join(missing_metrics)}")
    ...
    if quality_metrics.get("metricBasis") != METRIC_BASIS:
        raise FeedbackDatasetEvaluationCardValidationError("qualityMetrics metricBasis unsupported")
    ...
    if records_returned != metadata_records_returned:
        raise FeedbackDatasetEvaluationCardValidationError("datasetSummary.recordsReturned must match datasetMetadata.recordsReturned")
    if records_returned != records_evaluated:
        raise FeedbackDatasetEvaluationCardValidationError("recordsReturned must equal recordsEvaluated")
    validate_class_count_integrity(
        _required_non_negative_int(class_balance, "positiveClassCount"),
        _required_non_negative_int(class_balance, "negativeClassCount"),
        records_evaluated,
    ...
    if not isinstance(summary.get("warnings"), list):
        raise FeedbackDatasetEvaluationCardValidationError("warnings must be a list")
...
def _metric_object(raw: dict[str, Any], field: str) -> None:
    ...
    if not isinstance(value, dict):
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be a metric object")
    if set(value) != {"available", "reason", "value"}:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} metric shape unsupported")
    available = value["available"]
    metric_value = value["value"]
    reason = value["reason"]
    if not isinstance(available, bool):
        raise FeedbackDatasetEvaluationCardValidationError(f"{field}.available must be boolean")
    if available:
        try:
            metric_value = require_finite_number(metric_value, f"{field}.value")
        except JsonContractError as exc:
            raise FeedbackDatasetEvaluationCardValidationError(f"{field}.value must be numeric") from exc
        if metric_value < 0.0 or metric_value > 1.0:
            raise FeedbackDatasetEvaluationCardValidationError(f"{field}.value out of range")
        if reason is not None:
            raise FeedbackDatasetEvaluationCardValidationError(f"{field}.reason must be null when available")
    else:
        if metric_value is not None:
            raise FeedbackDatasetEvaluationCardValidationError(f"{field}.value must be null when unavailable")
        if not isinstance(reason, str) or not reason:
            raise FeedbackDatasetEvaluationCardValidationError(f"{field}.reason required when unavailable")


def _governance_metadata(raw: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(raw, dict):
        raise FeedbackDatasetEvaluationCardValidationError("governance metadata must be an object")
    _reject_forbidden_input(raw)
    extra = sorted(set(raw) - ALLOWED_GOVERNANCE_METADATA_FIELDS)
    if extra:
        raise FeedbackDatasetEvaluationCardValidationError(f"governance metadata contains unsupported fields: {', '.join(extra)}")
    missing = sorted(REQUIRED_GOVERNANCE_METADATA_FIELDS - set(raw))
    if missing:
        raise FeedbackDatasetEvaluationCardValidationError(f"governance metadata missing required fields: {', '.join(missing)}")
    return dict(raw)
...
def _warnings(summary: dict[str, Any]) -> list[str]:
    ...
    if not all(isinstance(item, str) for item in value):
        raise FeedbackDatasetEvaluationCardValidationError("warnings must contain strings")
    if len(set(value)) != len(value):
        raise FeedbackDatasetEvaluationCardValidationError("warnings contains duplicate values")
    return sorted(value)
...
def _required_object(raw: dict[str, Any], field: str) -> dict[str, Any]:
    value = raw.get(field)
    if not isinstance(value, dict):
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be an object")
    return value
...
def _reject_unknown(raw: dict[str, Any], allowed: set[str], label: str) -> None:
    ...
    if extra:
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} contains unsupported fields: {', '.join(extra)}")


def _required_non_negative_int(raw: dict[str, Any], field: str) -> int:
    value = raw.get(field)
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be a non-negative integer")
    return value


def _require_canonical_filename(path: Path, expected_name: str, label: str) -> None:
    if path.name != expected_name:
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} filename must be {expected_name}")


def _read_required_bytes(path: Path, label: str, max_bytes: int) -> bytes:
    _reject_symlink_path(path, label)
    if not path.is_file():
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} missing")
    flags = os.O_RDONLY | getattr(os, "O_BINARY", 0) | getattr(os, "O_NOFOLLOW", 0)
    try:
        fd = os.open(path, flags)
    except OSError as exc:
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} missing") from exc
    try:
        if not stat.S_ISREG(os.fstat(fd).st_mode):
            raise FeedbackDatasetEvaluationCardValidationError(f"{label} must be a file")
    ...
    if len(payload) > max_bytes:
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} exceeds maximum byte size")
    return payload


def _reject_symlink_path(path: Path, label: str) -> None:
    if path.is_symlink():
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} must not be a symlink")
    parent = path.parent
    while parent != parent.parent:
        if parent.is_symlink():
            raise FeedbackDatasetEvaluationCardValidationError(f"{label} parent must not be a symlink")
...
def _load_json_bytes(payload: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} must be valid JSON") from exception
    if not isinstance(value, dict):
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} must be a JSON object")
    return value
