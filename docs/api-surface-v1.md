...
- FDP-23 includes only a disabled external-anchoring hook for trust-authority audit heads. `app.trust-authority.external-anchoring.enabled=false` by default; enabling it logs the local head and calls the extension interface, but no real external publisher is supplied until FDP-24.
