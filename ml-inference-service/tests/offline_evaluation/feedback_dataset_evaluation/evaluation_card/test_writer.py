...
from offline_evaluation.feedback_dataset_evaluation.evaluation_card.run_evaluation_card_generation import main
...
from offline_evaluation.feedback_dataset_evaluation.evaluation_card.writer import (
...
class FeedbackDatasetEvaluationCardWriterTest(unittest.TestCase):
    ...
    def test_manifestIsWrittenLast(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            with patch("offline_evaluation.feedback_dataset_evaluation.evaluation_card.writer.os.replace", recording_replace):
    ...
    def test_cleansTempFilesOnFailure(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory)
            with patch("offline_evaluation.feedback_dataset_evaluation.evaluation_card.writer.os.replace", failing_replace):
    ...
    def test_interruptedOverwriteRemovesFinalManifestAndRejectsMixedDirectory(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("offline_evaluation.feedback_dataset_evaluation.evaluation_card.writer.os.replace", fail_after_json_replace):
                with self.assertRaises(OSError):
...
class FeedbackDatasetEvaluationCardCliTest(unittest.TestCase):
