...
Run the readiness and regression jobs locally:
...
- Modeled crash during evidence-gated finalize before commit.
- Modeled crash after local commit before external confirmation.
...
| Legacy recovery with snapshot is not committed success | `AlertControllerTest` | `recoveryRequiredWithSnapshot_mustNotReplayCommittedSuccess` | controller | `regulated-mutation-regression` | Recovery-required legacy state could replay success | API returns explicit recovery | Business commit not guaranteed |
...
| Long-running processing is observable through API | `AlertControllerTest` | `longRunningProcessing_mustBeObservableNotSuccess` | controller | `regulated-mutation-regression` | Processing state could look successful | API exposes processing | Business commit not guaranteed |
