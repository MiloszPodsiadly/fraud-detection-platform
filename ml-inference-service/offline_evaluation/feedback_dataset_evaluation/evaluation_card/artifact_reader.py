...
def _validate_manifest(
    ...
    if manifest.get("reportType") != PLATFORM_RECOMMENDATION_EVALUATION_CARD_REPORT_TYPE:
        raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest reportType unsupported")
    if manifest.get("artifactSetVersion") != ARTIFACT_SET_VERSION:
        raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest artifactSetVersion unsupported")
    try:
    ...
    if manifest.get("generatedAt") != card_generated_at:
        raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest generatedAt must match card generatedAt")
    files = manifest.get("files")
    if not isinstance(files, list) or len(files) != len(EXPECTED_ARTIFACT_FILENAMES):
        raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest files must list the canonical artifacts")
    seen_names: set[str] = set()
    for item in files:
        if not isinstance(item, dict):
            raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest file entries must be objects")
        ...
        if name not in EXPECTED_ARTIFACT_FILENAMES:
            raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest lists unsupported artifact")
        if name in seen_names:
            raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest contains duplicate artifact")
        seen_names.add(name)
        ...
        if not isinstance(sha256, str) or len(sha256) != 64 or not all(c in "0123456789abcdef" for c in sha256):
            raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest sha256 must be lowercase hex")
        if isinstance(size_bytes, bool) or not isinstance(size_bytes, int) or size_bytes < 0:
            raise FeedbackDatasetEvaluationCardValidationError("evaluation card manifest sizeBytes must be a non-negative integer")
        if sha256 != hashlib.sha256(expected_bytes).hexdigest():
            raise FeedbackDatasetEvaluationCardValidationError("evaluation card artifact hash mismatch")
        if size_bytes != len(expected_bytes):
            raise FeedbackDatasetEvaluationCardValidationError("evaluation card artifact size mismatch")
...
def _reject_unknown_or_missing(raw: dict[str, Any], allowed: set[str], label: str) -> None:
    ...
    if missing:
        raise FeedbackDatasetEvaluationCardValidationError(f"{label} missing required fields: {', '.join(missing)}")
