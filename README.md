...
6. If `regulated_mutation_recovery_required_count > 0`, inspect `GET /api/v1/regulated-mutations/by-command/{commandId}` or `GET /api/v1/regulated-mutations/by-idempotency-hash/{hash}` before retrying. The raw idempotency-key inspection path remains compatibility-only. Do not infer rollback from command state; use the command state and durable degradation records.
