...
- External anchor publication is disabled by default (`app.audit.external-anchoring.enabled=false`). Supported sink names are `disabled`, `local-file`, and `object-store`; the legacy `external-object-store` placeholder is rejected. When `app.audit.external-anchoring.enabled=true`, startup requires a verified external witness and rejects fake publishers including `disabled`, `noop`, `local-file`, `in-memory`, and `same-database`. Local-file is only a development verification artifact and cannot satisfy enabled external anchoring. FDP-22 `object-store` writes new anchors to a configured external object-store adapter using `audit-anchors/{encoded_partition_key}/{chain_position_padded}.json`, where `chain_position_padded` is a 20-digit zero-padded number and `encoded_partition_key` is deterministic base64url without padding. Legacy non-padded keys remain readable without a migration job. Object-store mode requires explicit bucket/prefix/region-or-endpoint/credentials/client configuration, performs startup readiness checking by default, and fails startup if configuration or readiness is incomplete. Enabled external anchoring requires verified read-after-write, stable reference, write-once behavior, retention/delete denial, and `external_immutability_level=ENFORCED`. External publication is not transactional with the durable Mongo audit insert and local anchor insert; if required external publication fails after those local records exist, the write path fails closed, records `LOCAL_ANCHOR_CREATED_EXTERNAL_REQUIRED_FAILED`, emits `external_anchor_required_failed_after_local_anchor_total`, and coverage exposes `local_ahead_of_external=true` plus `required_publication_failures`.
...
- Companion publication status records track external publication status fields for later operational inspection. Success records require `external_published=true`, `external_publication_status=PUBLISHED`, `external_published_at`, `external_sink_type`, `external_object_status`, `head_manifest_status`, `local_tracking_status`, and `local_anchor_status`; unverified records set `external_published=false`, `external_publication_status=UNVERIFIED`, and a bounded `last_external_publish_failure_reason`; failure records set `external_published=false`, `external_publication_status=FAILED`, and may include `last_external_publish_failure_reason`. If status persistence fails after external publication/read-back, `external_anchor_status_persistence_failed_total{sink}` is incremented and the write path fails closed in enabled mode with `STATUS_PERSISTENCE_FAILED_AFTER_EXTERNAL_PUBLISH`; the system must not report a clean local `PUBLISHED` state. If required external publication fails after local anchor creation, the status is `LOCAL_ANCHOR_CREATED_EXTERNAL_REQUIRED_FAILED` with `external_required=true`, `local_anchor_status=CREATED`, and `external_object_status=FAILED` or `UNAVAILABLE`. Reconciliation scans a bounded recent local window for missing status, `LOCAL_STATUS_UNVERIFIED`, and required-failure states, verifies exact `partition_key + chain_position`, and records recovered/still-unverified/missing/invalid results. These fields are not part of the event hash chain and updating them does not mutate audit events or local anchor records.
