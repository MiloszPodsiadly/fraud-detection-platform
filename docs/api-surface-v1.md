...
- Fraud-case update writes still use the FDP-24 guarded audit pattern. Submit decision writes now use the FDP-25 regulated mutation coordinator.
- `POST /api/v1/alerts/{alertId}/decision` requires `X-Idempotency-Key`, stores a `regulated_mutation_commands` record, writes durable `ATTEMPTED` audit before business mutation, persists the decision plus same-document outbox, then writes durable `SUCCESS` audit before returning `COMMITTED_EVIDENCE_PENDING`. The same key and payload replay the stored response snapshot; the same key with a different payload returns 409; a missing key returns 400.
- Analyst decision responses expose `operation_status`: `REJECTED_BEFORE_MUTATION`, `COMMITTED_EVIDENCE_PENDING`, or `COMMITTED_EVIDENCE_INCOMPLETE`. `REJECTED_BEFORE_MUTATION` maps to HTTP 503 with no command response body and means mutation was not executed. `COMMITTED_EVIDENCE_PENDING` means the decision and outbox record committed and local `SUCCESS` audit was recorded, while external evidence promotion remains pending. `COMMITTED_EVIDENCE_INCOMPLETE` means the decision and outbox record committed but local success audit/evidence completion degraded; it is not a rollback and is counted by durable `audit_degradation_events` plus `fraud_platform_post_commit_audit_degraded_total`. `COMMITTED_FULLY_ANCHORED` is async-only and intentionally not exposed by the synchronous request path.
- FDP-25 is a regulated commit protocol for the submit-decision mutation. It is not distributed ACID across MongoDB, external witnesses, and Kafka. External evidence remains asynchronous and reconciled; fraud-case update and governance writes remain future coordinator migrations.
