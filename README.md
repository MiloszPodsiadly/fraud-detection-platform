...
Trust incident reads are read-only. `GET /api/v1/trust/incidents` and `GET /system/trust-level` do not create or update incidents. Durable materialization is explicit through `POST /api/v1/trust/incidents/refresh`, protected by backend authority and bounded low-cardinality metrics. Active trust incident dedupe is atomic by active dedupe key; resolved or false-positive incidents clear that active key. FDP-26 supports `OPEN`, `ACKNOWLEDGED`, `RESOLVED`, and `FALSE_POSITIVE`; `MITIGATING` is not implemented in this scope.
