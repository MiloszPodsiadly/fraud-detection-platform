...
FDP-26A is a local Mongo ACID boundary plus transactional outbox model for submit-decision writes. `TransactionalOutboxRecordDocument` is authoritative outbox state; alert embedded outbox fields are projection/cache only and may be repaired later. Broker publication uses a durable `PUBLISH_ATTEMPTED` state so possible broker success never falls back to blind retry. Manual confirmation resolution requires `X-Idempotency-Key`, structured evidence, and a regulated mutation command. FDP-26A is not distributed ACID across MongoDB, external witnesses, trust-authority signing, and Kafka. It does not provide exactly-once delivery, pre-commit/finalize evidence-gated commit, legal notarization, WORM storage, SIEM integration, certified archive evidence, or rollback of already visible business state. Fraud-case update and governance writes remain future migrations to this coordinator.
