...
class PythonMlSignalEngineArchitectureDocsTest {
    ...
    void documentsAdapterOnlyBoundaryAndSafetyRules() throws Exception {
        ...
        assertThat(docs)
                ...
                .contains("missing model availability metadata is degraded")
                .contains("invalid model availability metadata is degraded")
                .contains("supported source reason codes")
                .contains("falls back to `ml_model_signal`")
                .contains("bounded ml reason codes")
                .contains("does not carry source scoringevidence directly")
                .contains("bounded reason codes only")
                .contains("future evidence mapping requires an explicit bounded evidence policy")
                .contains("programmer defects and other unexpected runtime exceptions")
                .contains("propagate to the orchestrator isolation boundary")
                .contains("known ml client/network boundary failures")
                .contains("bounded `ml_client_error`")
                .contains("available adapter output uses `unknown` confidence")
                .contains("confidence is not inferred from ml score")
