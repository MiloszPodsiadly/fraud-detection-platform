...
| `GET /api/v1/trust/incidents/signals/preview` | `trust-incident:read` | Read-only trust signal preview; per-actor/IP rate-limited and read-access audited; no materialization and no mutation audit. |
| `POST /api/v1/trust/incidents/refresh` | `trust-incident:refresh` or `trust-incident:resolve` | Regulated, idempotent trust signal materialization requiring `X-Idempotency-Key`; read endpoints do not write incidents. |
...
- `GET /system/trust-level` requires backend-enforced `audit:verify` and is live-state posture, not config-only posture. It returns `NONE`, `BEST_EFFORT`, `FDP24_CONFIGURED`, `FDP24_HEALTHY`, or `FDP24_DEGRADED` plus coverage, witness, signature policy, missing range, required-failure, local-status-unverified, post-commit audit degradation signals, pending degradation resolution count, outbox terminal/unknown counts, pending outbox resolution count, oldest pending/ambiguous outbox age, regulated mutation recovery-required count, stale processing lease count, committed-degraded count, repeated recovery failure count, oldest recovery-required age, and trust incident health fields. `FDP24_HEALTHY` requires enabled+required+fail-closed publication, `app.audit.bank-mode.fail-closed=true`, provider capability verified or stronger witness status, healthy bounded coverage, no required publication failures, no local status gaps, no unresolved post-commit audit degradation, no pending degradation resolution, no terminal or unknown outbox confirmation state, no pending outbox resolution, no regulated mutation backlog health signals, no open/unacknowledged critical trust incidents, and required signature health when configured. `PROVIDER_CAPABILITY_VERIFIED` is capability evidence, not notarization, TSA, mTLS, or live independent witness proof.
- FDP-25 Phase 1 introduces a regulated mutation coordinator for submit-decision writes and outbox unknown-confirmation reconciliation. FDP-26 extends it to fraud-case update and trust operations. FDP-29 adds a disabled-by-default `EVIDENCE_GATED_FINALIZE_V1` local evidence-precondition-gated finalize path for submit-decision only. The coordinator owns mandatory idempotency, command state, atomic command execution claim/lease, canonical intent fields for recovery validation, model-version tagging, idempotent phase audit request ids (`regulated_command_id:ATTEMPTED|SUCCESS|FAILED`), response snapshot replay, recovery-required states, and degraded evidence state. FDP-29 model-specific evidence preparation and finalize execution are isolated in `EvidenceGatedFinalizeExecutor`. FDP-26 adds the local Mongo transaction boundary and separate `transactional_outbox_records` source of truth for submit-decision local commits; embedded alert outbox fields remain compatibility/projection fields. FDP-29 submit-decision requires `app.regulated-mutations.transaction-mode=REQUIRED` and rejects before business mutation if the required local transaction boundary is unavailable. The v1 local gate checks attempted audit, deterministic success audit key availability, transaction/outbox/recovery capability, bounded local audit writer retry config, required local audit-chain unique indexes, actor/resource/action intent consistency, and submit-decision business validation before visible mutation; local success audit evidence is written through `RegulatedMutationLocalAuditPhaseWriter` inside the local finalize transaction without generic `AuditService` publisher fanout. External anchor readiness and Trust Authority signing readiness remain asynchronous/future target gates. Fraud-case update stores bounded canonical intent for case id, actor, action, status, assignee hash, notes hash, tags hash, and payload hash. Trust incident refresh stores a hash of bounded collected signals in the regulated command intent; the preview endpoint is read-only and does not create command state. Signal preview requires `trust-incident:read`, is per-actor/IP rate-limited, writes a bounded read-access audit, and fails closed in bank/prod mode if that audit cannot be persisted. In `transaction-mode=REQUIRED` or `app.trust-incidents.refresh-mode=ATOMIC`, refresh failures roll back and are not partial commits. Partial trust-incident refresh reporting is restricted to non-transactional/local `refresh-mode=PARTIAL` and must be verified from persisted active dedupe state. `PARTIAL` is local/dev/test only, not regulator-grade, and startup fails in prod-like/bank environments if it is configured. `IN_PROGRESS`, `RECOVERY_REQUIRED`, `COMMIT_UNKNOWN`, and FDP-29 preparation/finalizing states are explicit HTTP 202 command states. `COMMITTED_EVIDENCE_CONFIRMED` and `FINALIZED_EVIDENCE_CONFIRMED` require local commit marker, success audit, authoritative `PUBLISHED` transactional outbox evidence, and any configured external/signature evidence; `COMMITTED_FULLY_ANCHORED` is not returned by the synchronous request path. Recovery is bounded and backend-authorized. System trust degrades when regulated mutation recovery-required, stale-lease, committed-degraded, repeated-failure, outbox projection mismatch, publish-attempted, confirmation-unknown, or open critical trust incident signals exist. FDP-26/FDP-29 are local Mongo transaction and transactional outbox models, not distributed ACID across MongoDB, external witnesses, and Kafka. They do not provide exactly-once broker delivery or legal/compliance trust. External evidence remains asynchronous and reconciled. Governance writes remain future coordinator migrations.
...
- Companion publication status records track external publication status fields for later operational inspection. They are the single local index for audit read/export/coverage external status; audit read/export resolve this index by exact chain positions and local anchor ids, not sparse min/max windows. Missing status is `UNKNOWN`, not guessed as local-only or published. Source-of-truth hierarchy is external witness object, publication-status repository, local audit chain, then head manifest cache. Success records require `external_published=true`, `external_publication_status=PUBLISHED`, `external_published_at`, `external_sink_type`, `external_object_status`, `head_manifest_status`, `local_tracking_status`, and `local_anchor_status`; unverified records set `external_published=false`, `external_publication_status=UNVERIFIED`, and a bounded `last_external_publish_failure_reason`; failure records set `external_published=false`, `external_publication_status=FAILED`, and may include `last_external_publish_failure_reason`. If status persistence fails after external publication/read-back, `external_anchor_status_persistence_failed_total{sink}` is incremented and the write path fails closed in required mode with `STATUS_PERSISTENCE_FAILED_AFTER_EXTERNAL_PUBLISH`; the system must not report a clean local `PUBLISHED` state. Clients MUST NOT infer business effectiveness from `outcome` or `compensated` alone; they must use `business_effective_status` with `audit_evidence_status`. Audit read and evidence export expose `compensated`, `superseded_by_event_id`, backward-compatible `business_effective`, authoritative `business_effective_status`, `audit_evidence_status`, per-event `external_anchor_status`, `compensation_type`, and `related_event_id`; `ATTEMPTED` is false, `SUCCESS` means the business write committed, and `ABORTED_EXTERNAL_ANCHOR_REQUIRED` is false. Compensation may exist outside the current page/window, and read/export APIs enrich returned events with bounded compensation lookup. If required external publication fails after local anchor creation, the status is `LOCAL_ANCHOR_CREATED_EXTERNAL_REQUIRED_FAILED` with `external_required=true`, `local_anchor_status=CREATED`, and `external_object_status=FAILED` or `UNAVAILABLE`. FDP-24 is fail-closed and semantically truthful, but not single-transaction atomic across the business DB, audit DB, and external witness; local chain position may advance even when a request fails. Anchor failure after a committed `SUCCESS` is evidence failure (`audit_evidence_status=ANCHOR_REQUIRED_FAILED`), not a business rollback (`business_effective_status=TRUE`). Analyst decision submission exposes `REJECTED_BEFORE_MUTATION`, `IN_PROGRESS`, `RECOVERY_REQUIRED`, `COMMIT_UNKNOWN`, `COMMITTED_EVIDENCE_PENDING`, and `COMMITTED_EVIDENCE_INCOMPLETE`; `COMMITTED_FULLY_ANCHORED` is intentionally not exposed until a safe asynchronous promotion path exists. `REJECTED_BEFORE_MUTATION` returns HTTP 503 and no command response body. `IN_PROGRESS`, `RECOVERY_REQUIRED`, and `COMMIT_UNKNOWN` return HTTP 202 and must not be interpreted as completed success. Post-commit degradation is explicit committed-incomplete behavior and must not be described as mutation not executed. Operators should alert on `requiredPublicationFailures > 0`, durable unresolved `audit_degradation_events`, pending degradation resolutions, post-commit degraded metrics, outbox terminal/unknown states, pending outbox resolutions, inspect coverage, and use explicit bounded range reconciliation. Unresolved audit degradation GET access requires `audit:verify`; resolution requires `audit-degradation:resolve`, structured evidence, ATTEMPTED/SUCCESS/FAILED audit outcomes, and dual-control in bank fail-closed mode. `PUBLISH_CONFIRMATION_UNKNOWN` GET access requires `audit:verify`; resolution requires `decision-outbox:reconcile`, structured evidence, and broker-offset evidence for `PUBLISHED`. Retry keeps the same `eventId`/`dedupeKey`, and there is no blind auto retry. Default reconciliation scans a bounded recent local window for missing status, `LOCAL_STATUS_UNVERIFIED`, and required-failure states. Large or old gaps require explicit bounded range reconciliation by `partition_key`, `from_position`, `to_position`, and `limit`; reconciliation verifies exact `partition_key + chain_position` and records recovered/still-unverified/missing/invalid results. These fields are not part of the event hash chain and updating them does not mutate audit events or local anchor records.

- FDP-26 adds a local regulated mutation transaction boundary for supported writes when `app.regulated-mutations.transaction-mode=REQUIRED`. The local Mongo transaction covers command state, business state, response snapshot, and local commit marker; submit-decision also writes a separate authoritative `transactional_outbox_records` document. Prod-like/bank startup fails unless transaction mode is `REQUIRED`, trust incident refresh mode is `ATOMIC`, a transaction manager is present, transaction capability probe succeeds, transactional outbox repository is present, outbox publisher and recovery are enabled, outbox confirmation dual-control is enabled, and max attempts are positive. `OFF` remains the local/dev compatibility mode. `app.trust-incidents.refresh-mode=PARTIAL` is local/dev/test only and must not be used for bank-grade operation.
- FDP-26 broker delivery is a transactional outbox model, not direct request-path publication. The request path writes a durable outbox record; the publisher claims records with bounded leases, durably records `PUBLISH_ATTEMPTED` before broker publication, and emits at-least-once with stable `eventId`, `dedupeKey`, `mutationCommandId`, causation/correlation data, and schema version. Consumers must deduplicate by event id. Confirmation-unknown, publish-attempted, terminal, and projection mismatch states degrade trust until resolved.
- FDP-26 recovery endpoints are operational and bounded: `GET /api/v1/outbox/recovery/backlog` requires `outbox:inspect`, `POST /api/v1/outbox/recovery/run` requires `outbox:recover`, and `POST /api/v1/outbox/{eventId}/resolve-confirmation` requires `outbox:resolve`, `X-Idempotency-Key`, `reason`, actor identity, and structured evidence. Manual outbox confirmation resolution is operator-attested and not independently broker-verified. In bank fail-closed mode, it is dual-control: requester and approver must be distinct authenticated operators, and startup fails if dual-control is disabled. Outside bank mode, responses explicitly report single-control operator attestation. Responses expose safe metadata only and never return event payloads, customer/account/card data, transaction payloads, decision notes, raw exception text, or unbounded exports.
- FDP-26/FDP-29 evidence confirmation is asynchronous. A legacy command becomes `COMMITTED_EVIDENCE_CONFIRMED` and an FDP-29 command becomes `FINALIZED_EVIDENCE_CONFIRMED` only when local commit marker, success audit, authoritative `PUBLISHED` outbox evidence, and any configured external anchor/signature evidence are present. Missing success audit becomes committed-degraded or finalize-recovery-required; unpublished, ambiguous, or externally incomplete evidence remains pending or recovery-required.
- FDP-26 is `FDP-26 - Regulated Trust Operations & Transactional Outbox Foundation`. Inside the local Mongo transaction boundary, when `transaction-mode=REQUIRED`, the system commits command local state, supported business aggregate mutation, response snapshot, and local commit marker; submit-decision also commits `transactional_outbox_records`. Outside that local transaction, it coordinates `ATTEMPTED` audit before mutation, `SUCCESS` audit after local commit, Kafka publish, external anchor publication, signature verification, and evidence confirmation. FDP-26 is not distributed ACID across MongoDB, Kafka, external witnesses, and trust-authority signing. It does not provide exactly-once delivery, WORM storage, legal notarization, legal non-repudiation, SIEM integration, regulator-certified archive evidence, or rollback of already visible business state. It detects and exposes post-commit audit/evidence degradation; it does not eliminate it transactionally. FDP-29 adds a limited feature-flagged local evidence-precondition-gated finalize model for submit-decision only.
- `FDP-27 - Pre-Commit Finalize Model` is not implemented in FDP-26. Candidate future states are `PENDING_COMMAND`, `ATTEMPT_EVIDENCE_READY`, `COMMIT_EVIDENCE_READY`, `FINALIZING`, `COMMITTED_LOCAL`, `EVIDENCE_CONFIRMED`, and `REJECTED_EVIDENCE_UNAVAILABLE`.
...
- `outbox_publish_attempt_total{result}`
- `outbox_delivery_latency_seconds`
- `evidence_confirmation_pending_count`
- `evidence_confirmation_failed_total{reason}`
...
FDP-27 does not provide distributed ACID, exactly-once Kafka, WORM storage, legal notarization, KMS/HSM, SIEM, or a regulator-certified archive.
