...
- `POST /api/v1/alerts/{alertId}/decision` requires `X-Idempotency-Key`, stores a `regulated_mutation_commands` record, atomically claims command execution with a bounded lease, writes durable `ATTEMPTED` audit before business mutation, persists the decision plus same-document outbox, then writes durable `SUCCESS` audit before returning `COMMITTED_EVIDENCE_PENDING`. Phase audits use deterministic request ids `regulated_command_id:ATTEMPTED|SUCCESS|FAILED` so recovery can bind existing evidence without duplicating success audit events. The same key and payload replay the stored response snapshot; the same key with a different payload returns 409; a missing key returns 400.
...
- `POST /api/v1/regulated-mutations/recover` requires `regulated-mutation:recover` and runs bounded recovery. `GET /api/v1/regulated-mutations/recovery/backlog` requires `regulated-mutation:recover` or `audit:verify` and returns bounded recovery counts by state/action. Recovery never re-runs business mutation after `BUSINESS_COMMITTING`; uncertain or unreconstructable states become `RECOVERY_REQUIRED`, and system trust degrades while recovery-required commands exist.
- FDP-25 Phase 1 is a saga-style recoverable command state machine for the submit-decision mutation. It is not distributed ACID across MongoDB, external witnesses, and Kafka, and it does not transactionally eliminate post-commit audit degradation. External evidence remains asynchronous and reconciled; fraud-case update and governance writes remain future coordinator migrations.
...
- `GET /api/v1/decision-outbox/unknown-confirmations` requires `audit:verify`; `POST /api/v1/decision-outbox/unknown-confirmations/{alertId}/resolve` requires `decision-outbox:reconcile`. Unknown confirmation can be resolved only as `PUBLISHED` or `RETRY_REQUESTED`; `PUBLISHED` requires `BROKER_OFFSET` evidence or equivalent broker confirmation, and retry keeps the same event id and dedupe key.
