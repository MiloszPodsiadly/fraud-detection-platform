...
The manifest is not a source of truth. `latest()` verifies the manifest hash and then reads the referenced anchor before trusting it. If the manifest is missing, unreadable, tampered, points to a missing anchor, or disagrees with the referenced anchor, the system records low-cardinality manifest metrics and falls back to the full paginated scan. Manifest update happens only after anchor write and read-after-write verification. Manifest update failure leaves the anchor valid and returns partial publication status; the manifest can be recomputed from anchors.
