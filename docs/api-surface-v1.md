...
- `POST /api/v1/alerts/{alertId}/decision` requires `X-Idempotency-Key`, stores a `regulated_mutation_commands` record with canonical intent fields and an idempotency-key hash, atomically claims command execution with a bounded lease, writes durable `ATTEMPTED` audit before business mutation, persists the decision plus same-document outbox, then writes durable `SUCCESS` audit before returning `COMMITTED_EVIDENCE_PENDING`. Phase audits use deterministic request ids `regulated_command_id:ATTEMPTED|SUCCESS|FAILED` so recovery can bind existing evidence without duplicating success audit events. The same key and payload replay the stored response snapshot; the same key with a different payload returns 409; a missing key returns 400.
- Analyst decision responses expose `operation_status`: `REJECTED_BEFORE_MUTATION`, `IN_PROGRESS`, `RECOVERY_REQUIRED`, `COMMIT_UNKNOWN`, `COMMITTED_EVIDENCE_PENDING`, or `COMMITTED_EVIDENCE_INCOMPLETE`. `REJECTED_BEFORE_MUTATION` maps to HTTP 503 with no command response body and means mutation was not executed. `IN_PROGRESS`, `RECOVERY_REQUIRED`, and `COMMIT_UNKNOWN` map to HTTP 202 because the command is accepted but not safely reportable as a stable completed result. `COMMITTED_EVIDENCE_PENDING` means the decision and outbox record committed and local `SUCCESS` audit was recorded, while external evidence promotion remains pending. `COMMITTED_EVIDENCE_INCOMPLETE` means the decision and outbox record committed but local success audit/evidence completion degraded; it is not a rollback and is counted by durable `audit_degradation_events` plus `fraud_platform_post_commit_audit_degraded_total`. `COMMITTED_FULLY_ANCHORED` is async-only and intentionally not exposed by the synchronous request path.
- `POST /api/v1/regulated-mutations/recover` requires `regulated-mutation:recover` and runs bounded recovery. `GET /api/v1/regulated-mutations/recovery/backlog` requires `regulated-mutation:recover` or `audit:verify` and returns bounded recovery counts by state/action. `GET /api/v1/regulated-mutations/{idempotencyKey}` remains a compatibility inspection path; `GET /api/v1/regulated-mutations/by-command/{commandId}` and `GET /api/v1/regulated-mutations/by-idempotency-hash/{hash}` are the preferred inspection paths. All inspection endpoints require `regulated-mutation:recover` or `audit:verify`, are rate-limited per actor/IP, audit successful inspection access, and expose command state/evidence ids without payload dumps. Recovery never re-runs business mutation after `BUSINESS_COMMITTING`; it validates reconstructed state against stored canonical intent, uncertain or unreconstructable states become `RECOVERY_REQUIRED`, and system trust degrades while recovery-required, stale-lease, committed-degraded, or repeated-failure signals exist.
- FDP-25 Phase 1 is a saga-style recoverable command state machine for the submit-decision mutation. It is not a single atomic boundary across MongoDB, external witnesses, and Kafka, and it does not eliminate post-commit audit degradation. External evidence remains asynchronous and reconciled; fraud-case update and governance writes remain future coordinator migrations.
