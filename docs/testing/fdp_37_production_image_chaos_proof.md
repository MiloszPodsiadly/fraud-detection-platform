...
Status: branch evidence and production-like image chaos proof reference.
...
- `PRODUCTION_IMAGE_CONTAINER_KILL`: production-like `alert-service` Docker image/container kill proof.
...
The required test evidence above is the authoritative scenario matrix for FDP-37. It supersedes the former standalone
...
Use `docs/fdp/fdp_37_enablement_decision_checklist.md`. `READY_FOR_ENABLEMENT_REVIEW` is not production enablement.
