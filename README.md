...
FDP-25 Phase 1 introduces a regulated mutation coordinator for `POST /api/v1/alerts/{alertId}/decision`. The request must provide `X-Idempotency-Key`; the coordinator stores a `regulated_mutation_commands` record, validates global idempotency, atomically claims execution with a bounded lease, writes `ATTEMPTED` audit before business mutation, persists the decision plus same-document outbox, then writes `SUCCESS` audit before returning a committed response. The same idempotency key and payload replay the stored response snapshot; the same key with a different payload returns 409. Missing idempotency key returns 400. Kafka publication remains asynchronous from the durable outbox and at-least-once.
