...
def _prepare_output_dir(output_dir: Path, allow_output_root: Path | None) -> None:
    if output_dir.exists():
        if output_dir.is_symlink():
            raise FeedbackDatasetEvaluationCardValidationError("output directory must not be a symlink")
        if not output_dir.is_dir():
            raise FeedbackDatasetEvaluationCardValidationError("output path exists and is not a directory")
    if allow_output_root is not None:
        ...
        if resolved_output != resolved_root and resolved_root not in resolved_output.parents:
            raise FeedbackDatasetEvaluationCardValidationError("output directory is outside allowed output root")
    ...
    try:
        for final_path in tuple(payloads) + (manifest_path,):
            if final_path.is_symlink():
                raise FeedbackDatasetEvaluationCardValidationError(f"final artifact path must not be a symlink: {final_path.name}")
