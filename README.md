...
`tools/audit-verifier/audit-verifier.mjs` is a local offline verifier for exported evidence material and public keys. It verifies the export fingerprint, signed anchor material, and chain continuity. Partial chain ranges require predecessor boundary proof; otherwise the verifier downgrades the result and does not report `INDEPENDENTLY_VERIFIABLE`. It does not call alert-service, MongoDB, or an object store.
