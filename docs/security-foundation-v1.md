...
- Internal auth metrics are `fraud_internal_auth_success_total{source_service,target_service}`, `fraud_internal_auth_failure_total{target_service,reason}`, `fraud_internal_auth_replay_rejected_total{reason}`, and `fraud_internal_auth_token_age_seconds{reason}`.
