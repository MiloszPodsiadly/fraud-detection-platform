...
- `/sign` is caller-bound and purpose-bound. Callers must send a per-service HMAC-signed request; bounded service identity headers are included in the HMAC input and are not trusted by themselves. The default local policy permits `alert-service` to sign `AUDIT_ANCHOR` and gives `fraud-scoring-service` no signing purpose. Unknown callers, invalid HMAC credentials, inconsistent payload-hash replay, and unauthorized purposes are rejected and audited. Prod-like profiles require an explicit caller allowlist with per-caller non-default HMAC secrets; the implicit local allowlist is rejected.
- `/sign` is rate-limited per verified service identity per minute and records low-cardinality metrics: `trust_sign_requests_total{status}`, `trust_verify_requests_total{status}`, `trust_invalid_signature_total`, `trust_unknown_key_total`, `trust_replay_detected_total`, and `trust_rate_limit_exceeded_total`. Legacy `trust_authority_*` metric aliases remain emitted for dashboard continuity.
- The local trust authority supports a minimal key registry with `ACTIVE`, `RETIRED`, and `REVOKED` keys. Only `ACTIVE` signs new anchors; `RETIRED` verifies historical signatures; `REVOKED` and unknown keys fail verification. Verification enforces `signed_at` against `valid_from` and `valid_until`. Prod-like profiles reject default local HMAC credentials, require `signing-required=true`, and reject generated ephemeral keys. Per-service HMAC is a local-first service identity guard only; it is not mTLS, not channel binding, not enterprise IAM, and not a zero-replay guarantee.
