...
External audit anchoring is disabled by default with `app.audit.external-anchoring.enabled=false`. When enabled with `app.audit.external-anchoring.sink=local-file`, alert-service publishes local audit anchors to an append-only JSONL sink outside MongoDB. Publication is idempotent by `local_anchor_id`; duplicate publication does not overwrite the existing external anchor. External sink failures are logged and counted, but they do not block durable audit writes.
