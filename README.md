...
Platform audit writes use an insert-only repository contract and store `previous_event_hash`, `event_hash`, `hash_algorithm=SHA-256`, and `schema_version`. The chain is scoped by `source_service` and is intended to detect application-level tampering, reordering, or deletion where the affected predecessor/successor is inside the bounded verification window. Corrections must be represented as new audit events, not mutation of existing events.
...
The durable audit store is not WORM archive storage, not external notarization, not legal non-repudiation, not SIEM integration, not a long-term archival policy, not a regulator-ready evidence package, and not cryptographic signing with HSM/KMS. Metrics such as `fraud_platform_audit_events_persisted_total`, `fraud_platform_audit_persistence_failures_total`, `fraud_platform_audit_read_requests_total`, `fraud_platform_audit_integrity_checks_total`, `fraud_platform_audit_integrity_violations_total`, `fraud_platform_read_access_audit_events_persisted_total`, `fraud_platform_read_access_audit_persistence_failures_total`, `fraud_read_access_audit_actor_missing_total`, `fraud_internal_auth_success_total`, and `fraud_internal_auth_failure_total` are operational health signals only.
