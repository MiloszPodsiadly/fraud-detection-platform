...
- `GET /api/v1/audit/integrity/external/coverage` requires `audit:verify` and returns bounded operational coverage: `latest_local_position`, `latest_external_position`, `position_lag`, `time_lag_seconds`, `missing_ranges`, `truncated`, and `limit`. `truncated=true` means the bounded window cannot prove complete coverage; there is no unbounded scan or export.
