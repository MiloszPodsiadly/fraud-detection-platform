...
- FDP-25 Phase 1 introduces a regulated mutation coordinator for submit-decision writes and outbox unknown-confirmation reconciliation. FDP-26 extends it to fraud-case update and trust operations. The coordinator owns mandatory idempotency, command state, atomic command execution claim/lease, canonical intent fields for recovery validation, idempotent phase audit request ids (`regulated_command_id:ATTEMPTED|SUCCESS|FAILED`), response snapshot replay, recovery-required states, and degraded evidence state. FDP-26 adds the local Mongo transaction boundary and separate `transactional_outbox_records` source of truth for submit-decision local commits; embedded alert outbox fields remain compatibility/projection fields. Fraud-case update stores bounded canonical intent for case id, actor, action, status, assignee hash, notes hash, tags hash, and payload hash. Trust incident refresh stores a hash of bounded collected signals in the regulated command intent; the preview endpoint is read-only and does not create command state. Signal preview requires `trust-incident:read`, is per-actor/IP rate-limited, writes a bounded read-access audit, and fails closed in bank/prod mode if that audit cannot be persisted. In `transaction-mode=REQUIRED` or `app.trust-incidents.refresh-mode=ATOMIC`, refresh failures roll back and are not partial commits. Partial trust-incident refresh reporting is restricted to non-transactional/local `refresh-mode=PARTIAL` and must be verified from persisted active dedupe state. `PARTIAL` is local/dev/test only, not regulator-grade, and startup fails in prod-like/bank environments if it is configured. `IN_PROGRESS`, `RECOVERY_REQUIRED`, and `COMMIT_UNKNOWN` are explicit HTTP 202 command states. `COMMITTED_EVIDENCE_CONFIRMED` requires local commit marker, success audit, and authoritative `PUBLISHED` transactional outbox evidence; `COMMITTED_FULLY_ANCHORED` is not returned by the synchronous request path. Recovery is bounded and backend-authorized. System trust degrades when regulated mutation recovery-required, stale-lease, committed-degraded, repeated-failure, outbox projection mismatch, publish-attempted, confirmation-unknown, or open critical trust incident signals exist. FDP-26 is a local Mongo ACID boundary and transactional outbox model, not distributed ACID across MongoDB, external witnesses, and Kafka. It detects and exposes post-commit audit/evidence degradation but does not provide exactly-once broker delivery, pre-commit/finalize evidence-gated commit, or legal/compliance trust. External evidence remains asynchronous and reconciled. Governance writes remain future coordinator migrations.
...
- FDP-26 evidence confirmation is asynchronous. A command becomes `COMMITTED_EVIDENCE_CONFIRMED` only when local commit marker, success audit, authoritative `PUBLISHED` outbox evidence, and any configured external anchor/signature evidence are present. Missing success audit becomes committed-degraded; unpublished, ambiguous, or externally incomplete evidence remains pending or recovery-required.
- FDP-26 is `FDP-26 - Regulated Trust Operations & Transactional Outbox Foundation`. Inside the local Mongo transaction boundary, when `transaction-mode=REQUIRED`, the system commits command local state, supported business aggregate mutation, response snapshot, and local commit marker; submit-decision also commits `transactional_outbox_records`. Outside that local transaction, it coordinates `ATTEMPTED` audit before mutation, `SUCCESS` audit after local commit, Kafka publish, external anchor publication, signature verification, and evidence confirmation. FDP-26 is not distributed ACID across MongoDB, Kafka, external witnesses, and trust-authority signing. It does not provide exactly-once delivery, pre-commit/finalize evidence-gated commit, WORM storage, legal notarization, legal non-repudiation, SIEM integration, regulator-certified archive evidence, or rollback of already visible business state. It detects and exposes post-commit audit/evidence degradation; it does not eliminate it transactionally.
