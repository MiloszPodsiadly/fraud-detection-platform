...
| --- | --- | --- | --- |
| [Internal fraud scoring orchestrator](fraud_scoring_orchestrator.md) | FDP-89 internal-only orchestration foundation | Internal `FraudScoringOrchestrator`, deterministic adapter execution, bounded per-engine failure containment, and runtime isolation | Claiming production scoring migration, Kafka event/API/UI/projection integration, final decisioning, or public `engineResults[]` |
