...
- `MTLS_READY` remains a fail-closed compatibility boundary. FDP-18 mTLS is internal-only and does not implement automated rotation, cert-manager, Vault/KMS, external PKI automation, or enterprise certificate lifecycle management.
...
- Internal auth metrics are `fraud_internal_auth_success_total{source_service,target_service,mode}`, `fraud_internal_auth_failure_total{target_service,mode,reason}`, `fraud_internal_mtls_certificate_expiry_seconds{source_service,target_service}`, `fraud_internal_auth_replay_rejected_total{reason}`, and `fraud_internal_auth_token_age_seconds{reason}`.
