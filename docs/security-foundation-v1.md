...
- External anchor publication is disabled by default (`app.audit.external-anchoring.enabled=false`). Supported sink names are `disabled`, `local-file`, and `object-store`; the legacy `external-object-store` placeholder is rejected. When enabled with `app.audit.external-anchoring.sink=local-file`, local Mongo anchors are published idempotently to a local verification JSONL file sink outside MongoDB. FDP-22 `object-store` writes anchors to a configured external object-store adapter using `audit-anchors/{encoded_partition_key}/{chain_position}.json`, where `encoded_partition_key` is deterministic base64url without padding and the payload retains the original `partition_key`. Object-store mode requires explicit bucket/prefix/region-or-endpoint/credentials/client configuration, performs startup readiness checking by default, and fails startup if configuration or readiness is incomplete. Publication failure is observable through logs and low-cardinality metrics and does not block durable audit writes.
