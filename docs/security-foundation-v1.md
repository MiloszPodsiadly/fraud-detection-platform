...
- FDP-22 object-store anchoring provides external persistence outside MongoDB and a logical immutable boundary: the application does not expose delete/update paths, checks existing objects before write, treats identical content as idempotent, and fails on different content for the same key. Immutability depends on object-store infrastructure configuration such as object lock, retention policy, bucket policy, or versioning. FDP-22 does not provide a WORM guarantee by itself, legal notarization, legal non-repudiation, compliance certification, SIEM integration, or regulator-ready archive.
