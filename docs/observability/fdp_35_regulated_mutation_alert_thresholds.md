...
request hash, resource id, exception message, token, or path as metric labels.

Do not include command id, alert id, actor id, lease owner, idempotency key, request hash, resource id, exception
message, token, or path in metric labels.

Priority mapping: `P2` means warning requires triage during the active support window; `P1` means critical incident
response and incident-lead ownership.

Window notation: `5m` means 5 minutes, `10m` means 10 minutes, `15m` means 15 minutes, and escalation to rollback
review must happen by 30 minutes when recovery backlog or false-success ambiguity grows.
