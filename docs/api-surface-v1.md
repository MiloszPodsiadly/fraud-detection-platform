...
- `POST /api/v1/alerts/{alertId}/decision` requires `X-Idempotency-Key`, stores a `regulated_mutation_commands` record, atomically claims command execution with a bounded lease, writes durable `ATTEMPTED` audit before business mutation, persists the decision plus same-document outbox, then writes durable `SUCCESS` audit before returning `COMMITTED_EVIDENCE_PENDING`. The same key and payload replay the stored response snapshot; the same key with a different payload returns 409; a missing key returns 400.
