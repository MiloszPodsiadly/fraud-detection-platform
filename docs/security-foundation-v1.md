...
- FDP-26A recovery endpoints are operational and bounded: `GET /api/v1/outbox/recovery/backlog` requires `outbox:inspect`, `POST /api/v1/outbox/recovery/run` requires `outbox:recover`, and `POST /api/v1/outbox/{eventId}/resolve-confirmation` requires `outbox:resolve`, `X-Idempotency-Key`, `reason`, and structured evidence. Responses expose safe metadata only and never return event payloads, customer/account/card data, transaction payloads, decision notes, raw exception text, or unbounded exports.
...
- FDP-26A is not distributed ACID across MongoDB, Kafka, external witnesses, and trust-authority signing. It does not provide exactly-once delivery, pre-commit/finalize evidence-gated commit, WORM storage, legal notarization, legal non-repudiation, SIEM integration, regulator-certified archive evidence, or rollback of already visible business state. SUCCESS audit and external evidence remain outside the local Mongo transaction unless a future phase changes the commit model.
