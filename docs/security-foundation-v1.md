...
- FDP-22 object-store anchoring maintains `<partition_prefix>/head.json` as an External Head Manifest optimization. The manifest is deterministic JSON with a `manifest_hash`, is updated only after anchor write and read-after-write verification, and is never authoritative. Latest HEAD lookup verifies the manifest hash and referenced anchor before use; missing or invalid manifests fall back to full paginated scan. Manifest update failure does not invalidate the anchor and does not create a false HEAD.
