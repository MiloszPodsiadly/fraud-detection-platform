...
The ML OpenAPI reference is `docs/openapi/ml_inference_service.openapi.yaml`.
The alert-service platform/governance audit OpenAPI reference is `docs/openapi/alert_service.openapi.yaml`.
...
- Default reconciliation is a bounded head-window convenience and is not proof of complete old-gap recovery. Old gaps require explicit bounded range reconciliation by partition and chain-position range.
...
See `docs/api/api_error_contract.md` for field rules and non-leakage requirements.
