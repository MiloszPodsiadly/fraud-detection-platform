...
| Evaluation label | FDP-102 internal dataset label named `evaluationLabel`; it is an analyst-review evaluation signal, not ground truth, not a model training label, and not a payment or final decision. |
| Feedback label | A bounded analyst-review label concept for rule or model evaluation. In FDP-102 it maps `CONFIRMED_FRAUD` to `POSITIVE`, `MARKED_LEGITIMATE` to `NEGATIVE`, and inconclusive, needs-more-info, missing, or unknown decisions to `NON_TRAINING`; inconclusive and missing decisions are never negative. |
| Non-training label | A label value excluded from model-quality metrics. It is used when a decision is missing, unknown, inconclusive, or needs more information. |
