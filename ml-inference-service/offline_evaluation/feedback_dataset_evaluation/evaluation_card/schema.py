...
from offline_evaluation.feedback_dataset_evaluation.dataset_schema import (
    ...
    MAX_DATASET_RECORDS as MAX_FEEDBACK_DATASET_RECORDS,
)
from offline_evaluation.feedback_dataset_evaluation.evaluation_contract import (
...
def validate_evaluation_card(raw: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(raw, dict):
        raise FeedbackDatasetEvaluationCardValidationError("evaluation card must be an object")
    _reject_unsafe(raw)
    extra = sorted(set(raw) - REQUIRED_PLATFORM_RECOMMENDATION_EVALUATION_CARD_FIELDS)
    if extra:
        raise FeedbackDatasetEvaluationCardValidationError(f"evaluation card contains unsupported fields: {', '.join(extra)}")
    missing = sorted(REQUIRED_PLATFORM_RECOMMENDATION_EVALUATION_CARD_FIELDS - set(raw))
    if missing:
        raise FeedbackDatasetEvaluationCardValidationError(f"evaluation card missing required fields: {', '.join(missing)}")
...
def _evaluation_subject(raw: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise FeedbackDatasetEvaluationCardValidationError("evaluationSubject must be an object")
    extra = sorted(set(value) - REQUIRED_EVALUATION_SUBJECT_FIELDS)
    if extra:
        raise FeedbackDatasetEvaluationCardValidationError(f"evaluationSubject contains unsupported fields: {', '.join(extra)}")
...
def _evaluation_evidence(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise FeedbackDatasetEvaluationCardValidationError("evaluationEvidence must be an object")
    extra = sorted(set(value) - REQUIRED_EVALUATION_EVIDENCE_FIELDS)
    if extra:
        raise FeedbackDatasetEvaluationCardValidationError(f"evaluationEvidence contains unsupported fields: {', '.join(extra)}")
...
def _metrics_summary(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or not value:
        raise FeedbackDatasetEvaluationCardValidationError("metricsSummary must be a non-empty object")
    extra = sorted(set(value) - ALLOWED_METRICS_SUMMARY_FIELDS)
    if extra:
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary contains unsupported fields: {', '.join(extra)}")
    missing = sorted(REQUIRED_METRICS_SUMMARY_FIELDS - set(value))
    if missing:
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary missing required fields: {', '.join(missing)}")
    summary = {
...
def _metric_object(raw: dict[str, Any], field: str) -> dict[str, float | bool | str | None]:
    value = raw.get(field)
    if not isinstance(value, dict):
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field} must be a metric object")
    extra = sorted(set(value) - {"available", "value", "reason"})
    if extra:
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field} contains unsupported fields")
    if set(value) != {"available", "value", "reason"}:
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field} must contain available, value, reason")
    available = value["available"]
    metric_value = value["value"]
    reason = value["reason"]
    if not isinstance(available, bool):
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field}.available must be boolean")
    if available:
        try:
        ...
        if metric_value < 0.0 or metric_value > 1.0:
            raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field}.value must be in range 0.0..1.0")
        if reason is not None:
            raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field}.reason must be null when available")
        return {"available": True, "value": metric_value, "reason": None}
    if metric_value is not None:
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field}.value must be null when unavailable")
    if not isinstance(reason, str) or MACHINE_CODE_PATTERN.fullmatch(reason) is None:
        raise FeedbackDatasetEvaluationCardValidationError(f"metricsSummary.{field}.reason must be machine-code when unavailable")
...
def validate_class_count_integrity(positive_class_count: int, negative_class_count: int, records_evaluated: int) -> None:
    if positive_class_count + negative_class_count != records_evaluated:
        raise FeedbackDatasetEvaluationCardValidationError("positiveClassCount + negativeClassCount must equal recordsEvaluated")
...
def _required_constant(raw: dict[str, Any], field: str, expected: str) -> str:
    ...
    if value != expected:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be {expected}")
    return value
...
def _allowed_usage_modes(raw: dict[str, Any]) -> list[str]:
    ...
    if rejected:
        raise FeedbackDatasetEvaluationCardValidationError("allowedUsageModes contains unsupported value")
    return values
...
def _intended_use(raw: dict[str, Any]) -> list[str]:
    ...
    if rejected:
        raise FeedbackDatasetEvaluationCardValidationError("intendedUse contains unsupported value")
    return values
...
def _required_machine_code_superset(raw: dict[str, Any], field: str, required: set[str]) -> list[str]:
    ...
    if missing:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} missing required values: {', '.join(missing)}")
    return values
...
def _machine_code_list(raw: dict[str, Any], field: str, max_items: int) -> list[str]:
    ...
    if not isinstance(value, list) or not value:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be a non-empty list")
    if len(value) > max_items:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} exceeds maximum item count")
    result = []
    for item in value:
        if not isinstance(item, str) or MACHINE_CODE_PATTERN.fullmatch(item) is None:
            raise FeedbackDatasetEvaluationCardValidationError(f"{field} must contain machine-code strings")
        if len(item) > 256:
            raise FeedbackDatasetEvaluationCardValidationError(f"{field} contains oversized item")
        _reject_unsafe_value(item)
        result.append(item)
    if len(set(result)) != len(result):
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} contains duplicate values")
...
def _optional_machine_code_list(raw: dict[str, Any], field: str, max_items: int) -> list[str]:
    ...
    if not isinstance(value, list):
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be a list")
    if len(value) > max_items:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} exceeds maximum item count")
    result = []
    for item in value:
...
def _required_count(raw: dict[str, Any], field: str) -> int:
    ...
    if value < 0:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be non-negative")
    if value > MAX_COUNT_VALUE:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} exceeds maximum value")
    return value
...
def _sha256(raw: dict[str, Any], field: str) -> str:
    ...
    if re.fullmatch(r"[a-f0-9]{64}", value) is None:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be sha256 hex")
    return value
...
def _bounded_string(raw: dict[str, Any], field: str, max_length: int) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} must be a non-empty string")
    if len(value) > max_length:
        raise FeedbackDatasetEvaluationCardValidationError(f"{field} exceeds maximum length")
