...
| `GET /api/v1/regulated-mutations/{idempotencyKey}` | `regulated-mutation:recover` or `audit:verify` | Read-only command inspection without payload dump. |
...
- FDP-25 Phase 1 introduces a regulated mutation coordinator for submit-decision writes and outbox unknown-confirmation reconciliation. The coordinator owns mandatory idempotency, command state, atomic command execution claim/lease, idempotent phase audit request ids (`regulated_command_id:ATTEMPTED|SUCCESS|FAILED`), business decision persistence with same-document outbox, response snapshot replay, recovery-required states, and degraded evidence state. `IN_PROGRESS`, `RECOVERY_REQUIRED`, and `COMMIT_UNKNOWN` are explicit HTTP 202 command states. `COMMITTED_FULLY_ANCHORED` is async-only and not returned by the synchronous request path. Recovery is bounded and backend-authorized through `POST /api/v1/regulated-mutations/recover` (`regulated-mutation:recover`), `GET /api/v1/regulated-mutations/recovery/backlog` (`regulated-mutation:recover` or `audit:verify`), and read-only `GET /api/v1/regulated-mutations/{idempotencyKey}` (`regulated-mutation:recover` or `audit:verify`). The inspection response exposes state/evidence metadata only and never dumps payloads. System trust degrades when regulated mutation recovery is required. FDP-25 Phase 1 is a saga-style recoverable command state machine, not distributed ACID across MongoDB, external witnesses, and Kafka; it detects and exposes post-commit audit degradation but does not transactionally eliminate it. External evidence remains asynchronous and reconciled. Fraud-case update and governance writes remain future coordinator migrations.
