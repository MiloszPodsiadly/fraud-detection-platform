...
@Component
public class TrustIncidentPreviewRateLimiter {
    ...
    @Autowired
    public TrustIncidentPreviewRateLimiter(
            @Value("${app.trust-incidents.preview.max-per-minute:30}") int maxRequestsPerMinute
    ) {
        this(Clock.systemUTC(), maxRequestsPerMinute);
    }

    TrustIncidentPreviewRateLimiter(Clock clock, int maxRequestsPerMinute) {
        this.clock = clock;
