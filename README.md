...
`tools/external-anchor-verifier/external-anchor-verifier.mjs` is a local offline verifier for external anchor objects and optional local evidence/reference JSON files. It validates `anchor_id_version=1`, the deterministic anchor id formula, payload hash binding, reference conflicts, stale anchors, and timestamp semantics without calling alert-service, MongoDB, trust-authority, or an object-store API. `APP_OBSERVED` timestamps downgrade to `UNVERIFIED`; they are not external proof.
