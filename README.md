...
External audit anchoring is disabled by default with `app.audit.external-anchoring.enabled=false`. FDP-20 supports `disabled` and local development `local-file`; FDP-22 adds `object-store` as an object-store external anchor sink behind `ExternalAuditAnchorSink`. The legacy `external-object-store` placeholder is rejected; use `app.audit.external-anchoring.sink=object-store`. When `app.audit.external-anchoring.enabled=true`, startup requires a verified external witness and rejects fake publishers including `disabled`, `noop`, `local-file`, `in-memory`, and `same-database`. Local-file remains a development verification artifact only and cannot satisfy enabled external anchoring. Publication is idempotent by deterministic anchor payload binding; duplicate publication does not overwrite the existing external anchor. In enabled mode, request-path audit writes fail closed if required external publication, read-back verification, or local publication-status persistence fails. Reconciliation/reporting paths may report failed or unverified counts, but the write path does not silently downgrade a required external anchor failure.
...
`tools/external-anchor-artifact-verifier/external-anchor-artifact-verifier.mjs` is a local offline artifact verifier for exported or locally copied external anchor JSON and optional local evidence/reference JSON files. It validates `anchor_id_version=1`, the deterministic anchor id formula, payload hash binding, reference conflicts, stale anchors, and timestamp semantics without calling alert-service, MongoDB, trust-authority, or an object-store API. It rejects HTTP/HTTPS inputs with `REMOTE_WITNESS_UNSUPPORTED`; it is not a live witness retriever. `APP_OBSERVED` timestamps downgrade to `UNVERIFIED`; they are not external proof.
