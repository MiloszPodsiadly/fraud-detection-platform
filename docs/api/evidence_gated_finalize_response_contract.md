...
This is the response contract for the feature-flagged submit-decision local evidence-precondition-gated finalize path.
It is active only when both evidence-gated finalize flags are enabled for submit-decision.

Core rule:

> No endpoint returns an updated business resource when state is not `FINALIZED_EVIDENCE_PENDING_EXTERNAL` or stronger for new evidence-gated submit-decision commands.
...
`current_snapshot` is the pre-finalize/current committed state. `updated_resource` is present only for
`FINALIZED_EVIDENCE_PENDING_EXTERNAL` or `FINALIZED_EVIDENCE_CONFIRMED` on new evidence-gated submit-decision
commands. `FINALIZED_VISIBLE` is accepted only as a compatibility/repair replay state and is mapped to public
...
- New evidence-gated submit-decision commands durably persist `FINALIZED_EVIDENCE_PENDING_EXTERNAL` as the local-visible
  state inside the local Mongo transaction. `FINALIZED_VISIBLE` is retained only for compatibility and repair.
