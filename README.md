...
| `POST /api/v1/outbox/{eventId}/resolve-confirmation` | `outbox:resolve` |
...
- FDP-26 Local Transaction Boundary and Transactional Outbox: optional Mongo transaction boundary for regulated submit-decision local commits plus a separate durable transactional outbox source of truth and bounded recovery APIs.
...
FDP-26 does not provide distributed ACID across MongoDB/Kafka/external witnesses, exactly-once delivery, legal notarization, WORM storage, SIEM integration, rollback of already committed business state, or regulator-certified archive evidence. Prod-like/bank deployments fail startup unless transaction mode is `REQUIRED`, a transaction manager is configured, outbox publisher and recovery are enabled, and max attempts are positive. Local/dev quickstart keeps `OFF` by default.
