...
- FDP-26 adds a local regulated mutation transaction boundary for submit-decision writes when `app.regulated-mutations.transaction-mode=REQUIRED`. The local Mongo transaction covers command state, alert business state, a separate `transactional_outbox_records` document, response snapshot, and local commit marker. Prod-like/bank startup fails unless transaction mode is `REQUIRED`, a transaction manager is present, outbox publisher and recovery are enabled, and max attempts are positive. `OFF` remains the local/dev compatibility mode.
- FDP-26 broker delivery is a transactional outbox model, not direct request-path publication. The request path writes a durable outbox record; the publisher claims records with bounded leases and emits at-least-once with stable `eventId`, `dedupeKey`, `mutationCommandId`, causation/correlation data, and schema version. Consumers must deduplicate by event id. Confirmation-unknown and terminal states degrade trust until resolved.
- FDP-26 recovery endpoints are operational and bounded: `GET /api/v1/outbox/recovery/backlog` requires `outbox:inspect`, `POST /api/v1/outbox/recovery/run` requires `outbox:recover`, and `POST /api/v1/outbox/{eventId}/resolve-confirmation` requires `outbox:resolve` plus structured evidence. Responses expose safe metadata only and never return event payloads, customer/account/card data, transaction payloads, decision notes, raw exception text, or unbounded exports.
- FDP-26 evidence confirmation is asynchronous. A command becomes `COMMITTED_EVIDENCE_CONFIRMED` only when local commit marker, success audit, and authoritative `PUBLISHED` outbox evidence are present. Missing success audit becomes committed-degraded; unpublished or ambiguous outbox evidence remains pending or recovery-required.
- FDP-26 is not distributed ACID across MongoDB, Kafka, external witnesses, and trust-authority signing. It does not provide exactly-once delivery, WORM storage, legal notarization, legal non-repudiation, SIEM integration, regulator-certified archive evidence, or rollback of already visible business state.
