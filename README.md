...
Write-path audit atomicity is explicit: for protected analyst decision and fraud-case update actions, the durable audit write is attempted before the business repository save. If the durable audit write fails, the request fails before the business action is persisted. Analyst decision submission also persists the domain event into a durable same-document outbox before returning; Kafka publication is asynchronous and retryable, so Kafka outage does not create a business-write-without-domain-event split. `X-Idempotency-Key` is supported for analyst decision submission: the same key and same payload replay the prior response without duplicate audit/outbox mutation, while the same key with a different payload is rejected. This is not a Mongo multi-document transaction and does not claim rollback of already written side effects outside that guarded sequence.

Analyst decision submission returns explicit operation states. `REJECTED_BEFORE_MUTATION` is returned as HTTP 503 and means the guarded audit write failed before the business decision was persisted. `COMMITTED_EVIDENCE_PENDING` means the business decision and durable outbox record were persisted, but regulator-grade evidence completion has not been persisted yet. `COMMITTED_FULLY_ANCHORED` means the decision and durable outbox record were persisted and the guarded audit write completed. `COMMITTED_EVIDENCE_INCOMPLETE` means the business decision and durable outbox record were already committed, but post-commit evidence/audit anchoring degraded; clients must not treat this as a rollback or as complete evidence. The platform records durable unresolved degradation rows in `audit_degradation_events` and emits `fraud_platform_post_commit_audit_degraded_total{operation="SUBMIT_ANALYST_DECISION"}` for this state. When `app.audit.bank-mode.fail-closed=true`, command responses never return `COMMITTED_EVIDENCE_PENDING` or `COMMITTED_EVIDENCE_INCOMPLETE`; audit failure is surfaced as 503.
...
`GET /system/trust-level` exposes the current deployment posture from live signals, not config alone: `guarantee_level`, publication flags, fail-closed flag, `external_anchor_strength`, coverage status, witness status, signature policy, required publication failures, local status gaps, missing ranges, durable post-commit audit degradation count, outbox terminal/unknown count, oldest pending outbox age, and reason code. Allowed guarantee levels are `NONE`, `BEST_EFFORT`, `FDP24_CONFIGURED`, `FDP24_HEALTHY`, and `FDP24_DEGRADED`. Witness status is `DISABLED`, `UNAVAILABLE`, `DECLARED_CAPABLE`, or `PROVIDER_VERIFIED`; declared capabilities alone are not proof. `FDP24_HEALTHY` requires `publication.enabled=true`, `publication.required=true`, `publication.fail-closed=true`, `witness_status=PROVIDER_VERIFIED`, healthy bounded coverage, no required publication failures, no local status gaps, no durable unresolved post-commit audit degradation, no terminal/unknown outbox confirmation failures, no stale pending outbox work, and required signature policy health when configured. `BEST_EFFORT` and `FDP24_DEGRADED` must not be marketed as full fail-closed mode. Startup logs `FDP-24 FAIL-CLOSED MODE ACTIVE` only when required publication and fail-closed mode are both enabled.
