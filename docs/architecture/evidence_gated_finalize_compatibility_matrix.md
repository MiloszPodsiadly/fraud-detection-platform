# Evidence-Gated Finalize Compatibility Matrix

Evidence-gated finalize does not rename legacy command states in place. Existing statuses remain accepted for legacy replay and inspection.
...
- APIs may map legacy states to future response statuses but must not change legacy meaning.
