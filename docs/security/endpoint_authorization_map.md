...
| `GET /api/v1/transactions/scored/{transactionId}/engine-intelligence/feedback` | `TransactionAuthorizationRules` | `ENGINE_INTELLIGENCE_FEEDBACK_READ` | Protected transaction-scoped feedback read model. Feedback is governance/review only and cannot be read with transaction display or write authority alone. | Safe method only | `AuthorizationRulesCoverageTest`, `RouteCoverageAgainstMvcMappingsTest` |
