...
Analyst decision submission returns explicit operation states. `REJECTED_BEFORE_MUTATION` means the guarded audit write failed before the business decision was persisted. `COMMITTED_AUDIT_PENDING` means the business decision and durable outbox record were persisted, but full audit completion has not been persisted yet. `COMMITTED_FULLY_ANCHORED` means the decision and durable outbox record were persisted and the guarded audit write completed. `COMMITTED_AUDIT_INCOMPLETE` means the business decision and durable outbox record were already committed, but the post-commit audit/anchor step degraded; clients must not treat this as a rollback. The platform records `fraud_platform_post_commit_audit_degraded_total{operation="SUBMIT_ANALYST_DECISION"}` for this state.
