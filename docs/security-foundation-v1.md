...
- FDP-26 adds a local regulated mutation transaction boundary for supported writes when `app.regulated-mutations.transaction-mode=REQUIRED`. The local Mongo transaction covers command state, business state, response snapshot, and local commit marker; submit-decision also writes a separate authoritative `transactional_outbox_records` document. Prod-like/bank startup fails unless transaction mode is `REQUIRED`, a transaction manager is present, transaction capability probe succeeds, transactional outbox repository is present, outbox publisher and recovery are enabled, outbox confirmation dual-control is enabled, and max attempts are positive. `OFF` remains the local/dev compatibility mode.
