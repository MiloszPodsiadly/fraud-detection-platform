# Evidence-Gated Finalize Failure Windows

This table defines expected behavior for the feature-flagged submit-decision evidence-gated finalize implementation.
...
- New evidence-gated submit-decision commands durably persist `FINALIZED_EVIDENCE_PENDING_EXTERNAL` as the local-visible
  state. `FINALIZED_VISIBLE` is compatibility/repair state only.
