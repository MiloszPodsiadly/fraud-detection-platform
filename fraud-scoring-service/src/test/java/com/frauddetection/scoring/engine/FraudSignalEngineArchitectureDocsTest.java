...
class FraudSignalEngineArchitectureDocsTest {
    ...
    void documentsInternalInterfaceBoundaryAndRejectedIntegrationClaims() throws Exception {
        ...
        assertThat(docs)
                ...
                .contains("`fraudenginedescriptor.enginelanguage` must remain aligned with public `fraudengineresult.enginelanguage`")
                .contains("`fraudsignalengine.evaluate()` returns the internal metadata-free `fraudsignalevaluation`")
                ...
                .contains("`required` is descriptive only at the interface boundary")
                .contains("bounded internal orchestration warnings")
                .contains("no external fallback semantics")
                ...
                .contains("fdp-87 adds an isolated `rulebasedsignalengine`")
                .contains("not a spring component")
                .contains("is not wired into runtime scoring")
                ...
                .contains("no public `engineresults[]`")
                .contains("no api/ui");
