...
- `audit-trust-authority` is a separate local service. It owns the Ed25519 private signing key for FDP-23 local verification and exposes `POST /api/v1/trust/sign`, `POST /api/v1/trust/verify`, `GET /api/v1/trust/keys`, and actuator health.
...
- `GET /api/v1/audit/trust/keys` requires `audit:verify` and proxies bounded public key metadata: `key_id`, `algorithm`, `public_key`, `valid_from`, `valid_until`, and `status`. It never exposes private key material.
...
- If the authority is unavailable and signing is not required, publication can remain externally anchored with `signature_status=SIGNATURE_UNAVAILABLE`; no trust upgrade occurs. If signing is required, publication is recorded as partial with `SIGNATURE_FAILED`.
- Every `/sign` and `/verify` call is synchronously written to an append-only trust-authority audit sink before the response is returned. Audit write failure fails the request. The audit event stores bounded action, caller identity/service, purpose, payload hash, key id, result, reason code, and timestamp only; it does not store raw payloads, tokens, private keys, or secrets.
- `/sign` is caller-bound and purpose-bound. A shared local token alone is insufficient: callers must provide bounded service identity headers, and the authority checks the purpose allowlist. The default local policy permits `alert-service` to sign `AUDIT_ANCHOR` and gives `fraud-scoring-service` no signing purpose. Unknown callers and unauthorized purposes are rejected and audited. Prod-like profiles require an explicit caller allowlist with per-caller non-default tokens; the implicit local allowlist is rejected.
- `/sign` is rate-limited per caller identity per minute and records low-cardinality metrics: `trust_authority_sign_total{status}`, `trust_authority_verify_total{status}`, `trust_authority_invalid_signature_total`, `trust_authority_unknown_key_total`, `trust_authority_revoked_key_total`, and `trust_authority_rate_limit_total`.
- The local trust authority supports a minimal key registry with `ACTIVE`, `RETIRED`, and `REVOKED` keys. Only `ACTIVE` signs new anchors; `RETIRED` verifies historical signatures; `REVOKED` and unknown keys fail verification. Verification enforces `signed_at` against `valid_from` and `valid_until`. Prod-like profiles reject the default shared token, require `signing-required=true`, and reject generated ephemeral keys. Shared-token auth is a local development guard only; it is not mTLS, not channel binding, and not enterprise service identity.
