...
Reviewer-friendly current operations spec for `fraud-detection-platform`.
...
The stable ML API contract is documented in `docs/openapi/ml_inference_service.openapi.yaml` and summarized in `docs/api/api_surface_v1.md`. Error responses use the platform `timestamp/status/error/message/details` envelope; metrics labels and governance fields remain bounded so observability changes do not alter scoring semantics.
...
- `fraud_internal_mtls_handshake_failures_total`
  - Type: counter
  - Meaning: rejected internal mTLS certificate handshakes or certificate-presenting requests
  - Labels: `reason`
  - Bounded reasons: `EXPIRED_CERT`, `UNTRUSTED_CA`, `HOSTNAME_MISMATCH`, `MISSING_CERT`
- `fraud_internal_mtls_cert_expiry_seconds`
  - Type: gauge
  - Meaning: seconds until an internal mTLS certificate expires
...
Detailed contract and playbook: [ML Governance And Drift v1](../ml/ml_governance_drift_v1.md).
