...
def build_feedback_dataset_disagreement_report(
        records: tuple[FeedbackDatasetRecord, ...] | list[FeedbackDatasetRecord],
...
def _disagreement_types(record: FeedbackDatasetRecord) -> list[str]:
    types = []
...
def _type_counts(records: tuple[FeedbackDatasetRecord, ...] | list[FeedbackDatasetRecord]) -> dict[str, int]:
    counts = {
