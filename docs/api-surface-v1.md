...
- External anchoring startup validates witness capabilities: fake witnesses are rejected, prod-like profiles require explicit independent witness policy, timestamps must not be application-observed for proof, and `durability_guarantee` is exposed as a bounded declared capability rather than a legal/WORM claim.
