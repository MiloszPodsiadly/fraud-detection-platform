...
- `GET /system/trust-level` requires backend-enforced `audit:verify` and is live-state posture, not config-only posture. It returns `NONE`, `BEST_EFFORT`, `FDP24_CONFIGURED`, `FDP24_HEALTHY`, or `FDP24_DEGRADED` plus coverage, witness, signature policy, missing range, required-failure, local-status-unverified, post-commit audit degradation signals, pending degradation resolution count, outbox terminal/unknown counts, pending outbox resolution count, oldest pending/ambiguous outbox age, regulated mutation recovery-required count, stale processing lease count, committed-degraded count, repeated recovery failure count, and oldest recovery-required age. `FDP24_HEALTHY` requires enabled+required+fail-closed publication, `app.audit.bank-mode.fail-closed=true`, provider capability verified or stronger witness status, healthy bounded coverage, no required publication failures, no local status gaps, no unresolved post-commit audit degradation, no pending degradation resolution, no terminal or unknown outbox confirmation state, no pending outbox resolution, no regulated mutation backlog health signals, and required signature health when configured. `PROVIDER_CAPABILITY_VERIFIED` is capability evidence, not notarization, TSA, mTLS, or live independent witness proof.
- FDP-25 Phase 1 introduces a regulated mutation coordinator for submit-decision writes and outbox unknown-confirmation reconciliation. The coordinator owns mandatory idempotency, command state, atomic command execution claim/lease, canonical intent fields for recovery validation, idempotent phase audit request ids (`regulated_command_id:ATTEMPTED|SUCCESS|FAILED`), response snapshot replay, recovery-required states, and degraded evidence state. FDP-26A adds the local Mongo transaction boundary and separate `transactional_outbox_records` source of truth for submit-decision local commits; embedded alert outbox fields remain compatibility/projection fields. `IN_PROGRESS`, `RECOVERY_REQUIRED`, and `COMMIT_UNKNOWN` are explicit HTTP 202 command states. `COMMITTED_EVIDENCE_CONFIRMED` requires local commit marker, success audit, and authoritative `PUBLISHED` transactional outbox evidence; `COMMITTED_FULLY_ANCHORED` is not returned by the synchronous request path. Recovery is bounded and backend-authorized through `POST /api/v1/regulated-mutations/recover` (`regulated-mutation:recover`), `GET /api/v1/regulated-mutations/recovery/backlog` (`regulated-mutation:recover` or `audit:verify`), legacy/debug `GET /api/v1/regulated-mutations/{idempotencyKey}`, preferred `GET /api/v1/regulated-mutations/by-command/{commandId}`, and preferred `GET /api/v1/regulated-mutations/by-idempotency-hash/{hash}`. Inspection is rate-limited per actor/IP, fails closed if inspection access cannot be durably audited, and exposes state/evidence metadata, idempotency key hash, and masked idempotency key only; it never echoes the raw idempotency key. Operators must not paste raw idempotency keys in tickets, logs, runbooks, or dashboards. System trust degrades when regulated mutation recovery-required, stale-lease, committed-degraded, repeated-failure, outbox projection mismatch, publish-attempted, or confirmation-unknown signals exist. FDP-26A is a local Mongo ACID boundary and transactional outbox model, not distributed ACID across MongoDB, external witnesses, and Kafka; it detects and exposes post-commit audit/evidence degradation but does not provide exactly-once broker delivery, pre-commit/finalize evidence-gated commit, or legal/compliance trust. External evidence remains asynchronous and reconciled. Fraud-case update and governance writes remain future coordinator migrations.
...
- FDP-26A adds a local regulated mutation transaction boundary for submit-decision writes when `app.regulated-mutations.transaction-mode=REQUIRED`. The local Mongo transaction covers command state, alert business state, a separate authoritative `transactional_outbox_records` document, response snapshot, and local commit marker. Prod-like/bank startup fails unless transaction mode is `REQUIRED`, a transaction manager is present, transaction capability probe succeeds, transactional outbox repository is present, outbox publisher and recovery are enabled, and max attempts are positive. `OFF` remains the local/dev compatibility mode.
- FDP-26A broker delivery is a transactional outbox model, not direct request-path publication. The request path writes a durable outbox record; the publisher claims records with bounded leases, durably records `PUBLISH_ATTEMPTED` before broker publication, and emits at-least-once with stable `eventId`, `dedupeKey`, `mutationCommandId`, causation/correlation data, and schema version. Consumers must deduplicate by event id. Confirmation-unknown, publish-attempted, terminal, and projection mismatch states degrade trust until resolved.
- FDP-26A recovery endpoints are operational and bounded: `GET /api/v1/outbox/recovery/backlog` requires `outbox:inspect`, `POST /api/v1/outbox/recovery/run` requires `outbox:recover`, and `POST /api/v1/outbox/{eventId}/resolve-confirmation` requires `outbox:resolve`, `X-Idempotency-Key`, `reason`, actor identity, and structured evidence. Manual outbox confirmation resolution is operator-attested, not broker-verified and not dual-control in FDP-26A. Responses expose safe metadata only and never return event payloads, customer/account/card data, transaction payloads, decision notes, raw exception text, or unbounded exports.
- FDP-26A evidence confirmation is asynchronous. A command becomes `COMMITTED_EVIDENCE_CONFIRMED` only when local commit marker, success audit, authoritative `PUBLISHED` outbox evidence, and any configured external anchor/signature evidence are present. Missing success audit becomes committed-degraded; unpublished, ambiguous, or externally incomplete evidence remains pending or recovery-required.
- FDP-26A is `FDP-26A - Local Transactional Outbox & Evidence Confirmation Foundation`. Inside the local Mongo transaction boundary, when `transaction-mode=REQUIRED`, the system commits command local state, business aggregate mutation, `transactional_outbox_records`, response snapshot, and local commit marker. Outside that local transaction, it coordinates `ATTEMPTED` audit before mutation, `SUCCESS` audit after local commit, Kafka publish, external anchor publication, signature verification, and evidence confirmation. FDP-26A is not distributed ACID across MongoDB, Kafka, external witnesses, and trust-authority signing. It does not provide exactly-once delivery, pre-commit/finalize evidence-gated commit, WORM storage, legal notarization, legal non-repudiation, SIEM integration, regulator-certified archive evidence, or rollback of already visible business state. It detects and exposes post-commit audit/evidence degradation; it does not eliminate it transactionally.
- `FDP-26B / FDP-27 - Pre-Commit Finalize Model` is not implemented in FDP-26A. Candidate future states are `PENDING_COMMAND`, `ATTEMPT_EVIDENCE_READY`, `COMMIT_EVIDENCE_READY`, `FINALIZING`, `COMMITTED_LOCAL`, `EVIDENCE_CONFIRMED`, and `REJECTED_EVIDENCE_UNAVAILABLE`. `FDP-28 - Trust Incident & Operations Control Plane` is not implemented in FDP-26A; candidate scope includes `TrustIncidentDocument`, dedupe fingerprint, `OPEN`, `ACK`, `MITIGATING`, `RESOLVED`, `FALSE_POSITIVE`, authorities, and durable lifecycle.
