...
They are not production WORM storage and are not suitable for high-volume production retention. The local-file sink reads the whole JSONL file for verification queries and is blocked in prod-like profiles (`prod`, `production`, `staging`). Production-grade external anchoring requires a future object-store/WORM sink.
