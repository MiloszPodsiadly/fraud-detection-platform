...
- FDP-23 Local-First Trust Authority: a separate local service signs external audit anchor hashes with asymmetric key material not held by alert-service and exports public verification keys.
...
- Every `/sign`, `/verify`, audit-integrity, and audit-head call is synchronously written to a trust-authority audit sink before the response is returned. Audit write failure fails the request. `local-file` audit is local/dev/test verification only. The durable sink is `durable-hash-chain`, which persists a bounded Mongo-backed hash chain with `previous_event_hash`, `event_hash`, and monotonic `chain_position`. Trust-authority audit storage labels are `LOCAL_FILE`, `DURABLE_HASH_CHAIN`, and future `EXTERNAL_WORM`; only externally enforced storage controls can justify `EXTERNAL_WORM`. The durable hash chain is tamper-evident, not immutable; a privileged database operator can rewrite history if hashes are recomputed. Concurrent append conflicts are resolved by one fresh-head retry, prioritizing correctness over availability. `GET /api/v1/trust/audit/integrity` verifies this chain and returns `VALID`, `INVALID`, or `UNAVAILABLE` with bounded violations. `GET /api/v1/trust/audit/head` returns `chain_position`, `event_hash`, and `timestamp` for future external anchoring; it does not publish to object store, blockchain, SIEM, or any external system. The audit event stores bounded action, caller identity/service, purpose, payload hash, key id, result, reason code, timestamp, and hash-chain metadata only; it does not store raw payloads, tokens, private keys, or secrets.
