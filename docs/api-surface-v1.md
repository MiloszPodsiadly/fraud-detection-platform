...
- `POST /api/v1/regulated-mutations/recover` requires `regulated-mutation:recover` and runs bounded recovery. `GET /api/v1/regulated-mutations/recovery/backlog` requires `regulated-mutation:recover` or `audit:verify` and returns bounded recovery counts by state/action. `GET /api/v1/regulated-mutations/{idempotencyKey}` requires `regulated-mutation:recover` or `audit:verify` and exposes command state/evidence ids without payload dumps. Recovery never re-runs business mutation after `BUSINESS_COMMITTING`; uncertain or unreconstructable states become `RECOVERY_REQUIRED`, and system trust degrades while recovery-required commands exist.
